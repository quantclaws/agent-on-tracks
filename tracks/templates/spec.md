...
>
> **交付入口（必读）**：每个有效 FR 必须标注用户触达该需求的产品表面。这不是实现细节，而是合同的一部分——验收时通过该入口确认需求已交付。入口形式取决于项目类型：页面路由（`/setup`）、CLI 命令（`trac init`）、API 端点（`POST /api/releases`）、输出文件（`dist/report.pdf`）、库函数（`parse(config)`）等。没有 UI 的项目同样适用。纯内部约束写“无独立入口，依附 FR-XXXX”。
...
- **交付入口**：{用户触达本需求的产品表面，例如 `/setup`、`trac init`、`POST /api/releases`；多个 FR 共享时写“同 FR-XXXX”}
