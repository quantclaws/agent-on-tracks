...
class FakeBackend(DevonPatchMixin, FakeShieldMixin):
    ...
    def _devon_commands_valid(value: object) -> bool:
        ...
        if isinstance(value, dict):
            return bool(value) and all(
                FakeBackend._devon_commands_valid(item) for item in value.values()
            )
        if isinstance(value, (list, tuple)):
            return bool(value) and all(FakeBackend._devon_commands_valid(item) for item in value)
        return False
    ...
    def _devon_manifest_paths_valid(value: object) -> bool:
        return (
            isinstance(value, (list, tuple))
            and bool(value)
            and all(FakeBackend._devon_manifest_path_valid(item) for item in value)
        )
