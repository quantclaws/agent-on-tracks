...
5. 用 `assignment.commands` 提供的 unit 命令运行授权 unit test，看到目标失败。**失败形态 = M-IMPL 合法分类**：全部失败节点必须一致分类为 `assertion_failure` 或 `symbol_missing`（RED_GATE 的合法集合；`stub_token_failure` 是 M-TEST 冻结资产的词汇，M-IMPL RED 中为**非法**——run 01M2QTJB T-002 三连拒实证）。装配错误（collection/import/fixture）同样非法。对被测面仍是 `raise NotImplementedError("IF-…")` 桩的（Shield 为 M-TEST 可读性预植的接口桩）：**用守卫把桩触发转成真断言**——`try: result = fn(...)\nexcept NotImplementedError:\n    raise AssertionError("IF-… 仍是桩：合同要求已实现行为") from None`，traceback 携带 `AssertionError` token，节点诚实落为 `assertion_failure`（"桩必须被行为替换"这一红线本身，兼作回退到桩的回归针；v0.8 先例 `test_t002_issue_mapping_red.py`："no stub_token, no assembly errors"）。**禁止用 `pytest.fail(...)` 做守卫**：它产出 `Failed:` 行，无 AssertionError token，分类器判 `unclassified`——非法（run 01M2QTJB T-003 2026-09-20 三次 attempt 实证：flash 通道照本宣科写出 pytest.fail 守卫被 RED_GATE 拒）。`assert False, "…"` 与 `raise AssertionError(…)` 等价合法。evidence 的 `results` 分类如实自报，runtime 只信不伪（DEVON-RED-5）。
