...
version: 0.1
description: D-29 M-DESIGN 评审判据包 - Prism 在 M-DESIGN DRAFT 评审消费的语义判据
---

# D-29 M-DESIGN 评审判据包

Prism 在 M-DESIGN PRISM_REVIEW 按 assignment 指定的名称+版本加载本判据包，审 Archer 起草的设计三件套（architecture、interfaces、test-plan）。本判据包只含**语义判据**（需求/AC/三向闭包、六元组闭合、Test Plan 可执行性、宿主项目测试执行合同、评审清单、架构与接口、可实现性、合同真实性、脚手架宣言），不含形式校验规则（template 合规、ID 文法、binding 完整性归 Runtime 程序校验，D-14 边界）。
...
三向闭包存在 orphan 或语义不一致的判 revise，经 `trac discuss` 锚定线程指出哪条 AC/IF/ARC 路径缺失或不一致。

## 判据 2：六元组闭合（ISLAND_GATE_1 输入）

每条 required AC 检查 owner / surface / composition / wiring / test / evidence 六项在设计文档中可定位：

- 任一缺失，或模块无法从任何交付面到达 → REVISE。
- 六元组 evidence 列为泛词（无具体命令/可观察输出）→ REVISE（亦见合同真实性）。
...
## 判据 4：宿主项目测试执行合同（`.tracks/project/project.toml`）
...
作者自证的评审清单（文档中出现作者勾选的 review checklist）直接判 revise（亦见合同真实性）。
...
- binding 完整性（AC↔IF↔ARC trace）：Runtime trace 工具（FR-0080）。
- discussion_ready 门禁：Runtime trace 工具。

Prism 的语义判据与 Runtime 的形式校验互补：Prism 可放行语义合格的设计三件套，但 template/trace 不合规的情况仍由 EXIT gate 复跑捕获（BS-06 恢复路径）。
