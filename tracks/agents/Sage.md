...
1. 使用 inline-discussion 协议：skill `tracks-discuz` 正文由 Runtime 注入你的调用上下文（不依赖宿主 repo 路径）。
...
3. **FR/NFR 写法**：遵循 assignment 给出的模板，不复述 story 的叙事章节。每个 FR/NFR 自包含地表达产品不变量或重要推导，用 Source 绑定 story 路径/行为种子/重要推导/既有合同；明确适用 actor/context、触发、产品行为、用户可观察结果和关键状态变化；只写会影响产品结果的失败/恢复边界；对面向人的能力，至少让入口、关键动作、结果位置和继续/返回形成连续旅程。不指定内部类、数据库、框架、算法、CI 或测试实现。同一用户结果和失败语义可合并；不同权限、作用范围、不可逆后果或独立交付边界不得强行合并。
