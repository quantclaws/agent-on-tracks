...
def _coverage_columns(cells: list[str]) -> tuple[int, int]:
    layer_idx = next((i for i, cell in enumerate(cells) if _LAYER_HEADER.search(cell)), None)
    if_idx = next((i for i, cell in enumerate(cells) if _IF_HEADER.search(cell)), None)
    return (layer_idx if layer_idx is not None else -1, if_idx if if_idx is not None else -1)
...
def _is_canonical_header(cells: list[str]) -> bool:
    ...
    if len(cells) < 4:
        return False
    return bool(
        _AC_COL.match(cells[0])
        and _LAYER_COL.match(cells[1])
        and _TEST_COL.match(cells[2])
        and _IF_COL.match(cells[3])
    )
...
def _scan_line_for_empty_check(
    stripped: str, in_coverage: bool, test_idx: int, header_seen: bool
...
def _check_empty_test_cells(plan_text: str, issues: list) -> None:
    ...
    for line in _visible_plan_lines(plan_text):
        ...
        if _table_separator(cells):
            continue
        if not header_seen:
            test_idx = next((i for i, cell in enumerate(cells) if _TEST_HEADER.search(cell)), -1)
            header_seen = True
            continue
...
def _parse_ac_row(
    ...
    return (sorted(candidate_layers), [] if malformed_if else sorted(if_ids))
...
def _rows_for_ac(
    ...
    return [(layer, if_cell) for row_ac, layer, if_cell in rows if row_ac == ac_id]
...
def required_ac_ids(acc_path, plan_path) -> set[str]:
    ...
    return {
        ac_id
        for ac_id, _ref, _ln, _sec in acs
...
def _shield_task_issues(
    ...
    if not task["if_ids"]:
        ...
        return out
    if if_registry is not None:
        out += [f"{if_id} not registered" for if_id in task["if_ids"] if if_id not in if_registry]
    return out


def _check_ac_task(ac_id: str, rows: list, if_registry: set[str] | None, issues: list) -> bool:
    ...
    return bool(
        task["layers"]
        and task["if_ids"]
        and (if_registry is None or all(i in if_registry for i in task["if_ids"]))
    )
...
def check_test_tasks(acc_text: str, plan_text: str, if_registry: set[str] | None = None) -> list:
    ...
    if not valid_shield:
        if any(_row_declares_shield(layer) for _ac, layer, _if in rows):
            ...
            issues.append("no valid Shield test task (no integration/e2e layer declared)")
    return issues
