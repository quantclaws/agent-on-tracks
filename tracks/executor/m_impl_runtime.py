...
def _task_review_failure(repo, run_id, events, task, g_sha, base_sha,
                         trailers, allowed, task_id, attempt) -> dict | None:
    diff_proc = git(Path(repo), "diff", "--name-only", base_sha, g_sha,
                    check=False)
    if diff_proc.returncode == 0:
        outside = sorted(path for path in diff_proc.stdout.splitlines()
                         if path.strip() and not any(
                             _manifest_path_matches(path.strip(), rule)
                             for rule in allowed))
        if outside:
            return {"check": "scope",
                    "reason": "G changes outside manifest allowed paths: "
                    + ", ".join(outside),
                    "evidence": json.dumps(outside)}
    proof = verify_lineage(repo, run_id, task_id, attempt, g_sha, events,
                           issue_number=task.issue_number,
                           ac_refs=_combined_provenance(task))
    if not proof.g_trailers_valid:
        return {"check": "lineage",
    ...
    if not (trailers.get("Tracks-AC") or "").strip():
        return {"check": "provenance",
                "reason": "G has no Tracks-AC provenance trailer"}
...
def _m_impl_red_classification_error(outcome: dict) -> tuple[str, str] | None:
    ...
    verdict = (_red_classification_label(outcome.get("verdict"))
               if verdict_present else "missing")
    evidence = json.dumps(
...
class MImplRuntimeMixin:
    ...
    def _materialize_m_impl_assignment(
        self, state: State, params: dict, command_id: str,
    ) -> dict:
        ...
        self._add_assignment_runtime_fields(
            assignment, state, params, task, pre_dirty,
        )
        assignment["test_tasks"] = parse_test_tasks(
            self._vdir() / "acceptance.md", self._vdir() / "test-plan.md",
        )
    ...
    def _materialized_task(self, state: State) -> dict | None:
        if not state.current_task_id:
            return None
        task_node = next(
            (self._task_node(raw) for raw in state.task_refs
             if raw.get("task_id") == state.current_task_id), None,
        )
        return self._task_payload(task_node) if task_node is not None else None

    def _add_assignment_runtime_fields(
        self, assignment: dict, state: State, params: dict,
        task: dict | None, pre_dirty: dict[str, str],
    ) -> None:
        role = params.get("role")
        substate = params.get("substate")
        task_id = task.get("task_id") if isinstance(task, dict) else "none"
        manifest = (dict(state.current_manifest)
                    if state.current_manifest is not None else None)
        result_identity = self._result_identity(
            task_id, str(substate or "dispatch").lower(),
            state.current_attempt, pre_dirty, state.taskgraph_digest or "",
        )
        if manifest is not None:
            manifest.update({"pre_dirty_snapshot": pre_dirty,
                             "result_identity": result_identity})
            if substate in ("GREEN", "REFACTOR", "PRISM_RED", "PRISM_FINAL",
                            "DIAGNOSE"):
        ...
        if task:
            for key in ("issue_number", "ac_refs", "fr_refs", "if_ids",
                        "test_refs"):
                value = task.get(key)
    ...
    def _add_assignment_role_fields(
        assignment: dict, state: State, params: dict,
    ) -> None:
        role = params.get("role")
        substate = params.get("substate")
        if role == "devon":
    ...
    def _invalid_m_impl_assignment(
        self, role: str, substate: str, assignment: dict | None,
    ) -> str | None:
        if role == "devon":
    ...
    def _invalid_devon_assignment(assignment: dict | None) -> str | None:
        required = ("task_id", "if_ids", "ac_refs", "test_refs",
                    "commands", "manifest", "phase",
                    "pre_dirty_snapshot", "result_identity")
        data = assignment or {}
        missing = [key for key in required
                   if MImplRuntimeMixin._assignment_key_missing(data, key)]
    ...
    def _assignment_key_missing(assignment: dict, key: str) -> bool:
        ...
        required_list = key in ("if_ids", "ac_refs", "test_refs",
                                "commands", "manifest")
    ...
    def _lookup_task(self, task_id: str):
        ...
        if (state.current_task_metadata
                and state.current_task_metadata.get("task_id") == task_id):
    ...
    def _task_node(raw: dict) -> TaskNode:
        return TaskNode(
            task_id=raw["task_id"], issue_number=raw["issue_number"],
            description=raw["description"], ac_refs=tuple(raw["ac_refs"]),
            fr_refs=tuple(raw["fr_refs"]), if_ids=tuple(raw["if_ids"]),
            test_refs=tuple(raw["test_refs"]),
            scope_boundary=raw["scope_boundary"],
            depends_on=tuple(raw["depends_on"]), batch=raw["batch"],
    ...
    def _devon_evidence_fields_error(phase: str, outcome: dict) -> str | None:
        required = ("phase", "changed_paths", "commands",
        ...
        if outcome.get("pre_identity") in (None, "") \
                or outcome.get("post_identity") in (None, ""):
    ...
    def _devon_evidence_change_error(phase: str, outcome: dict) -> str | None:
        ...
        if phase == "refactor" and not outcome.get("changed_paths") \
    ...
    def _do_freeze_baseline(self, cmd, state, task_id, reconcile):
        ...
        digest = m_impl_baseline_digest(
            vdir, self.repo, approval_digest=approval_digest,
            issue_evidence=issue_evidence, frozen_test_paths=frozen,
            branch=branch, tip=tip, design_checkpoint=design_checkpoint,
        )
        try:
        ...
        missing = list(m_impl_baseline_missing(
            vdir, self.repo, approval_digest=approval_digest,
            issue_evidence=issue_evidence, frozen_test_paths=frozen,
            contract_valid=contract_valid,
            branch=branch, tip=tip, design_checkpoint=design_checkpoint,
        ))
        self._emit("baseline.frozen",
                   {"status": "current" if not missing else "stale",
                    "digest": digest, "summary": m_impl_baseline_summary(vdir),
                    "frozen_test_paths": frozen, "missing": missing,
    ...
    def _do_commit_taskgraph(self, cmd, state, task_id, reconcile):
        ...
        self._emit("taskgraph.committed",
                   {"task_count": len(tasks),
                    "task_ids": [t.task_id for t in tasks],
                    "tasks": [self._task_payload(t) for t in tasks],
                    "path": "tasks.json", "validate_status": "pass",
                    "digest": hashlib.sha256(raw.encode("utf-8")).hexdigest(),
                    "tasks_md": "tasks.md"}, command_id=cmd.command_id)
        self._rebuild_task_log_projection()
    ...
    def _emit_taskgraph_failure(
        self, cmd, state, reason: str, evidence: str = "",
    ) -> None:
        self._emit("verdict.failed",
                   {"check": "taskgraph", "reason": reason,
                    "evidence": evidence, "attempt": state.current_attempt + 1},
                   command_id=cmd.command_id)
    ...
    def _taskgraph_errors(vdir: Path, tasks: list[TaskNode]) -> list[str]:
        ...
        ac_ids, if_registry, doc_errors = MImplRuntimeMixin._taskgraph_docs(
            vdir / "acceptance.md", vdir / "interfaces.md",
        )
    ...
    def _task_payload(task: TaskNode) -> dict:
        return {
            "task_id": task.task_id, "issue_number": task.issue_number,
            "description": task.description, "ac_refs": list(task.ac_refs),
            "fr_refs": list(task.fr_refs), "if_ids": list(task.if_ids),
            "test_refs": list(task.test_refs), "scope_boundary": task.scope_boundary,
            "depends_on": list(task.depends_on), "batch": task.batch,
            "parallel": task.parallel, "budget": task.budget,
        }
    ...
    def _tasks_md(tasks) -> str:
        ...
        for task in tasks:
            lines.extend([
                f"## {task.task_id}", f"- Issue: #{task.issue_number}",
                ...
                f"- Batch: {task.batch}", f"- Parallel: {task.parallel}",
    ...
    def _do_check_island_1(self, cmd, state, task_id, reconcile):
        ...
        if errors:
            self._emit("verdict.failed",
                       {"check": "island", "reason": "; ".join(errors),
                        "evidence": "\n".join(errors),
                        "attempt": state.current_attempt + 1},
                       command_id=cmd.command_id)
            return
        self._emit("verdict.passed", {"check": "island_1"},
                   command_id=cmd.command_id)

    def _island_taskgraph_errors(
        self, vdir: Path, tasks_path: Path, state: State,
    ) -> list[str]:
        ...
        if error is not None:
            return [error]
        if state.taskgraph_digest and hashlib.sha256(
                raw.encode("utf-8")).hexdigest() != state.taskgraph_digest:
        ...
        if not arch_path.is_file():
            ...
            ok, closure = validate_island_closure(
                tasks, arch_path.read_text(encoding="utf-8"),
            )
            if not ok:
    ...
    def _do_check_island_2(self, cmd, state, task_id, reconcile):
        ...
        if reach.status != "pass":
            self._emit("verdict.failed",
                       {"check": "island",
                        "reason": "reach check failed: "
                        + "; ".join((*reach.errors, *reach.islands)),
                        "evidence": str(reach),
                        "attempt": state.current_attempt + 1},
                       command_id=cmd.command_id)
            return
        ...
        if error is not None:
            self._emit("verdict.failed",
                       {"check": "full_suite", "reason": error,
                        "evidence": error,
                        "attempt": state.current_attempt + 1},
                       command_id=cmd.command_id)
            return
        failures = [f"{name}: " + _short_detail(stdout + "\n" + stderr)
                    for name, code, stdout, stderr in results if code != 0]
        if failures:
            self._emit("verdict.failed",
                       {"check": "full_suite", "reason": "; ".join(failures),
                        "evidence": "; ".join(failures),
                        "attempt": state.current_attempt + 1},
                       command_id=cmd.command_id)
            return
        self._emit("verdict.passed", {"check": "island_2"},
                   command_id=cmd.command_id)
    ...
    def _do_select_task(self, cmd, state, task_id, reconcile):
        ...
        ready = [task for task in tasks
                 ...
                 and all(dep == "-" or dep in completed
    ...
    def _start_task(self, cmd, task: TaskNode, manifest: dict) -> None:
        payload = {"task_id": task.task_id,
                   "task": self._task_payload(task), "manifest": manifest}
        self._emit("writelock.granted",
                   {**payload, "allowed_paths": self._task_allowed_paths(task),
    ...
    def _parse_allowed_paths(scope_boundary: str) -> list[str]:
        ...
        return sorted({p.rstrip("/") for p in parts
    ...
    def _test_commands(self) -> dict:
        try:
            ...
            return {}
        return {name: section.run
                for name, section in (("integration", contract.integration),
                                      ("e2e", contract.e2e))
        ...
        if not baseline_digest:
            for ev in reversed(list(self.store.events(self.run_id))):
                if ev.type == "baseline.frozen":
                    ...
                    break
        manifest = {
            "task_id": task.task_id, "task_ref": self._task_payload(task),
            "issue_number": task.issue_number, "if_ids": list(task.if_ids),
            "ac_refs": list(task.ac_refs), "fr_refs": list(task.fr_refs),
            "test_refs": list(task.test_refs), "scope_boundary": task.scope_boundary,
            ...
            "budget": task.budget, "baseline_identity": baseline_digest,
            "unit_commands": self._unit_commands(),
            "test_commands": self._test_commands(),
            "guard_commands": self._guard_commands(),
            "pre_dirty_snapshot": pre_dirty, "r_tree_identity": None,
            "result_identity": None,
        }
        manifest["result_identity"] = self._result_identity(
            task.task_id, "manifest", state.current_attempt, pre_dirty,
            baseline_digest,
        )
        return manifest
    ...
    def _result_identity(
        self, task_id: str, phase: str, attempt: int,
        pre_dirty: dict[str, str], baseline_digest: str,
    ) -> str:
        material = json.dumps(
            {"run_id": self.run_id, "task_id": task_id, "phase": phase,
             "attempt": attempt, "pre_dirty": pre_dirty,
    ...
    def _ensure_gate_worktree(self, state: State) -> tuple[str, WorktreeHandle | None]:
        ...
        gate_path = os.path.join(str(self.repo), ".tracks", "worktrees",
                                 self.run_id, task_id, "gate")
        ...
        try:
            handle = create_gate_worktree(
                str(self.repo), base_sha, r_sha, g_diff,
                self.run_id, task_id,
            )
    ...
    def _r_tests_in_working_tree(self, r_sha: str) -> bool:
        ...
        proc = git(self.repo, "diff-tree", "--no-commit-id", "--name-only",
                   "-r", r_sha, check=False)
        if proc.returncode != 0:
            return False
        rels = [line.strip() for line in proc.stdout.splitlines()
                if line.strip().startswith("tests/")]
        return bool(rels) and all(
            os.path.isfile(os.path.join(str(self.repo), rel)) for rel in rels
        )

    def _emit_gate_failure(self, cmd, *, check, reason, task_id, attempt,
                           evidence=""):
        self._emit("verdict.failed",
                   {"check": check, "reason": reason, "evidence": evidence,
    ...
    def _run_green_gate(self, cmd, state: State) -> None:
        ...
        if reason is not None:
            self._emit_gate_failure(cmd, check="impl_defect", reason=reason,
        ...
        try:
            for command in self._unit_commands_from_manifest():
                ...
                if obs.exit_code != 0:
                    self._emit_gate_failure(cmd, check="impl_defect",
            ...
            if r_sha and r_sha.strip() and r_sha != "0" * 40:
                proc = git(Path(cwd), "diff", "--name-only", r_sha, "--", "tests/",
                           check=False)
                if proc.returncode == 0 and proc.stdout.strip():
                    self._emit_gate_failure(cmd, check="regression",
                                            reason="Runtime observed R unit tests changed",
                                            evidence=json.dumps(
                                                {"r_sha": r_sha, "changed_tests":
                                                 sorted(set(proc.stdout.splitlines()))}),
                                            task_id=task_id, attempt=attempt)
                    return
            self._emit("verdict.passed", {"check": "green"},
                       command_id=cmd.command_id)
        finally:
    ...
    def _run_task_review_gate(self, cmd, state: State) -> None:
        task_id = state.current_task_id
        attempt = state.current_attempt + 1
        events = [{"seq": ev.seq, "type": ev.type, "payload": dict(ev.payload)}
                  for ev in self.store.events(self.run_id)]
        green = next((ev for ev in reversed(events)
                      if ev["type"] == "green.committed"), None)
        ...
        if green is None or task is None or not g_sha or not base_sha:
            self._emit_gate_failure(cmd, check="lineage",
        ...
        failure = _task_review_failure(
            str(self.repo), self.run_id, events, task, g_sha, base_sha,
            payload.get("trailers") or {}, manifest.get("allowed_paths") or [],
            payload.get("task_id") or task_id, payload.get("attempt") or attempt)
        if failure is not None:
            self._emit_gate_failure(cmd, check=failure["check"],
                                    reason=failure["reason"],
                                    evidence=failure.get("evidence", ""),
                                    task_id=task_id, attempt=attempt)
            return
        self._emit("verdict.passed", {"check": "task_review"},
                   command_id=cmd.command_id)
    ...
    def _emit_task_gate(self, cmd, state, phase, failure_check, pass_check):
        ...
        if reason is None and phase == "red" and state.stage == "M-IMPL":
            ...
            if classification_error is not None:
                ...
                self._emit(
                    "verdict.failed",
                    {"check": failure_check, "reason": reason,
                     "evidence": evidence, "task_id": state.current_task_id,
                     "attempt": state.current_attempt + 1},
                    command_id=cmd.command_id, task_id=state.current_task_id,
                )
                return
        if reason is not None:
            self._emit("verdict.failed",
                       ...
                       command_id=cmd.command_id, task_id=state.current_task_id)
            return
        self._emit("verdict.passed", {"check": pass_check},
                   command_id=cmd.command_id)

    def _m_impl_event_recorded(
        self, event_type: str, task_id: str, attempt: int,
    ) -> bool:
        return any(
            ev.type == event_type
    ...
    def _do_checkpoint_red(self, cmd, state, task_id, reconcile):
        task_id = (state.current_task_id or cmd.params.get("task_id")
                   or task_id or "")
        attempt = state.current_attempt + 1
        ...
        if reason is not None:
            self._emit("verdict.failed",
                       {"check": "red_invalid", "reason": reason,
                        "evidence": "backend Devon outcome",
                        "task_id": task_id, "attempt": attempt},
                       command_id=cmd.command_id, task_id=task_id)
            self._rebuild_task_log_projection()
        ...
        r = create_red_ref(
            repo=str(self.repo), run_id=self.run_id,
            task_id=task_id, attempt=attempt, test_diff=diff, base_sha=base_sha,
        )
        self._emit("red.checkpointed",
                   {"ref": r.ref, "r_sha": r.sha, "task_id": task_id,
                    "attempt": attempt},
                   command_id=cmd.command_id, task_id=task_id)
        self._rebuild_task_log_projection()
    ...
    def _do_commit_green(self, cmd, state, task_id, reconcile):
        task_id = (state.current_task_id or cmd.params.get("task_id")
                   or task_id or "")
        attempt = state.current_attempt + 1
        if any(ev.type == "verdict.failed" and ev.payload.get("check") == "impl_defect"
        ...
        if reason is not None:
            self._emit("verdict.failed",
                       {"check": "impl_defect", "reason": reason,
                        "evidence": "backend Devon outcome",
                        "task_id": task_id, "attempt": attempt},
                       command_id=cmd.command_id, task_id=task_id)
            self._rebuild_task_log_projection()
        ...
        g = create_green_commit(
            repo=str(self.repo), run_id=self.run_id,
            task_id=task_id, attempt=attempt, impl_diff=diff, base_sha=b_sha,
            r_sha=r_sha, issue_number=task.issue_number,
            ac_refs=_combined_provenance(task),
        )
        ...
        if not materialized:
            self._emit("verdict.failed",
                       {"check": "impl_defect", "reason": materialize_reason,
                        "task_id": task_id, "attempt": attempt,
        ...
        self._emit("green.committed",
                   {"g_sha": g.sha, "task_id": task_id, "attempt": attempt,
                    "r_sha": r_sha, "base_sha": b_sha,
                    "trailers": g.trailers},
                   command_id=cmd.command_id, task_id=task_id)
        self._rebuild_task_log_projection()

    def _materialize_green_commit(
        self, g_sha: str, b_sha: str,
    ) -> tuple[bool, str | None]:
    ...
    def _do_run_refactor_gate(self, cmd, state, task_id, reconcile):
        ...
        try:
            ...
            if reason is not None:
                self._emit("verdict.failed",
                           {"check": "regression", "reason": reason,
                            "evidence": "backend Devon outcome",
                            "attempt": state.current_attempt + 1},
                           command_id=cmd.command_id)
                return
            ...
            if outside:
                self._emit("verdict.failed",
                           {"check": "scope",
                            "reason": "refactor changed forbidden paths: "
                            + ", ".join(outside), "evidence": str(outside),
                            "attempt": state.current_attempt + 1},
                           command_id=cmd.command_id)
                return
            git(self.repo, "add", "--", *changed)
            proc = _commit_if_staged(
                self.repo, f"M-IMPL: refactor\n\ncommand_id: {cmd.command_id}",
            )
            if proc is None:
                self._emit("verdict.failed",
                           {"check": "regression",
            ...
            if proc.returncode != 0:
                ...
                return
            self._emit("refactor.committed", {"task_id": state.current_task_id},
                       command_id=cmd.command_id)
        finally:
            if gate_handle is not None:
                cleanup_worktree(gate_handle)
    ...
    def _do_complete_task(self, cmd, state, task_id, reconcile):
        ...
        already_completed = any(
            ev.type == "task.completed" and ev.payload.get("task_id") == tid
            for ev in events
        )
        if already_completed:
            released = any(
                ev.type == "writelock.released"
                and ev.payload.get("task_id") == tid for ev in events
            )
            if not released:
                self._emit("writelock.released", {"task_id": tid},
                           command_id=cmd.command_id)
            self._rebuild_task_log_projection()
            return
        self._emit("task.completed", {"task_id": tid},
                   command_id=cmd.command_id)
        self._emit("writelock.released", {"task_id": tid},
                   command_id=cmd.command_id)
        self._rebuild_task_log_projection()
