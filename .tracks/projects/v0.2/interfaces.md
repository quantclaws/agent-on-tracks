...
IF-001 §7 `Verdict.check` Literal 追加三个成员：
...
- `template`：`validate_document(checks=["template"])` → 按 `tracks/templates/<kind>.md` 校验必备 frontmatter 字段 + level-2 章节（模板的 HTML 注释忽略；acceptance 的 level-2 章节随 FR/NFR 变化故不做章节名匹配），`evidence` 含不符项 `line:N`。spec 文档另过结构 lint（`check_spec_items`：`### FR-XXXX 标题` + `- **来源**：` + FR 的 `- **交付入口**：`；FR-20 scope 数所有 FR 条目，废弃项删除而非标记）。
