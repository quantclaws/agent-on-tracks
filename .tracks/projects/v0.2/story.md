...
| 文档模板          | tracks/templates/ 有模板但未接入；cmd_start 用 main.py 硬编码 STORY_TEMPLATE；validate_document 是 D-16 直通 | 模板接入 runtime；校验合同：M-START 骨架不校验 / outcome 即校验 / 门禁再校验 / trac validate |
...
- **用户起点**：`trac run` 调 Scribe/Sage 生成文档；评审阶段与门禁校验文档。
- **入口/触发**：生成文档时套用模板；门禁处校验格式。

1. M-START 创建空骨架：从 tracks/templates/ 读取对应模板（取代硬编码），套模板、保留占位符，不校验。
2. Scribe/Sage outcome 完成：必须产出规范文档，Runtime 立即校验；不合格 → 不进入评审、走重派。
...
- **Human 决定**（含 Aaron §5.1 批注细化）：区分"空骨架创建"与"Agent outcome"——M-START 空骨架不校验；Scribe/Sage 每次生成必须产出规范文档、Runtime 生成后即校验；评审退出门禁再次强制校验；独立 `trac validate` 命令可单独跑。
