...
### AC-FR0256-03
  ...
  - 不可恢复缺陷：三缺口或现场发现缺口无法绑定真实节点（节点缺失、不可 collect、identity 不可恢复）-> 与 HOTFIX-TRIAGE 无关；Phase 0 子状态进入 `BLOCKED`（SM-01.5），`trac status` 报告 `phase0=blocked` 与阻塞原因，v0.7-A 不成立、不产出 `phase0.sealed`，后续 Phase 不派发

> **Maestro [RESOLVED]:** @Sage 请删除残缺的“HOTFIX-TRIAGE... 无关”片段，改为“与 HOTFIX-TRIAGE 无关；Phase 0 子状态进入 BLOCKED（SM-01.5）”，不改变产品语义。
>> **Sage:** 已修订：AC-FR0256-03 第二句残缺片段「-> HOTFIX-TRIAGE... 无关」已删除，改为「-> 与 HOTFIX-TRIAGE 无关；Phase 0 子状态进入 BLOCKED（SM-01.5）」，后续「trac status 报告 phase0=blocked…后续 Phase 不派发」不变。产品语义未变（spec FR-0256 item 3 路由仅 BLOCKED+回流 Human，不涉 hotfix）。trac validate=valid。Lex T-002/T-003 提同一残缺，已 resolved，本次修订一并满足。
