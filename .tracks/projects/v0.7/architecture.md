...
- 新增 `kernel/phase0.py` 与 `executor/phase0.py`：M-DESIGN EXIT 后、首次 M-TEST 派发前运行 SM-01；扫描并真实绑定 v0.6 trace 缺口（三缺口节点从未落盘，`node_missing` 经 BLOCKED→Human 批准 baseline-repair→re-validate 编排补齐，§1.0.2 第 3 条），执行覆盖率/守卫/环境校验，封存 v0.6。
...
- 【R 修订（第二次 lineage 回滚重入，B91）】本 run 第二次以同一证据形态（check=lineage，"G trailers do not match the immutable R lineage"）重回 M-DESIGN DRAFT。根因是 Runtime 侧 RGR 台账的双重语义缺陷而非设计或实现缺陷：SM-01.14（`_on_test_committed`）在 test_defect 轮后刻意把 `state.r_tree_identity` 重指到获准的 Shield fix commit（避免被读作 drift），使后续 G commit 的 `Tracks-R` trailer 继承了 fix commit sha（T-013：`Tracks-R=5b72700` vs slot-4 ref `d826d34`）——R/G refs 不可变，任何 verify_lineage 重试都确定性复败。路由与第一次重入相同（B57：awaiting=rollback → Human 批准丢弃 M-IMPL 进度 → `trac recover` 重回 M-DESIGN）；Runtime 修正 B91（f638075）把 lineage 锚点改为经该 task 的 `red.checkpointed` 家族解析：精确 `r_sha` 匹配保持权威，非匹配身份（Shield 覆写）回退到最新 checkpoint `r_sha`，无 checkpoint 一律 fail-closed——重入后的 M-IMPL 重规划/重放不再命中该确定性死点，且该修正收紧而非放宽 lineage 校验。设计影响收敛为本修订的合同级增量：lineage 锚点解析语义（精确 r_sha 匹配权威 / Shield 覆写回退最新 checkpoint r_sha / 无 checkpoint 一律 fail-closed）从实现事实提升为设计合同，冻结于 interfaces §5 IF-IMPL-004 的 v0.7 扩展（继承标识符的合同增补，不新增 IF）；test-plan §1.1/§10 同步其可观察出口与既有 integration 回归行。除此之外，模块边界、EVENT_TYPES/COMMAND_KINDS 封闭集、IF 标识符集合、§8 覆盖表、Scaffold 宣言与 registry 本版全部不变；§4.2 全部 config_digest 已按当前仓库 bytes 复核一致（pyproject=898db7a1…、.flake8=f899995c…、project.toml=5ccdc0ea…；demo 0e05b693…/3c05476e…/103a0041…）。IF-IMPL-007 schema v2 层分家合同继续有效，重入后的 M-IMPL PLANNING 仍按其执行。
...
1. 扫描 v0.6 全部 approved AC 的 test-plan outlet、实际 collect inventory 与执行证据。planned binding 的唯一来源是 baseline 版本 test-plan §8 的 AC→node 绑定行（机器解析）；已知三缺口 AC-FR0250-03、AC-NFR0130-01、AC-NFR0130-02@v0.6 的 planned node 固定为 `tests/integration/test_evidence_reuse.py::test_prism_consumes_runtime_evidence_without_suite_rerun`、`::test_identity_fields_append_only_auditable`、`::test_stale_propagation_uniform_across_gates`——v0.6 test-plan §8 声明而从未落盘的节点（`git log --all -S` 对该文件全历史为空），即缺口成因。三节点在当前仓库不存在是首次 `phase0_validate` 的预期输入事实：绑定仍必须经 adapter `run_selected` 取得真实 node id、node digest、selection/evidence identity，SHALL NOT 以 marker/条目形式闭合。
2. marker-only 不产生 binding；不可 collect、节点缺失或 identity 不可恢复发出 `phase0.blocked(reason=node_missing|node_uncollectable|identity_unrecoverable)`，`detail` 按 IF §1a 枚举缺失 planned node id；后续 M-TEST 不派发，run 回流 Human（SM-01.5）。
3. **BLOCKED→Human 修复→re-validate 编排（node_missing 的修复通道合同）**：`node_missing` 不是终态死点，而是 §1c 已冻结的重入路径的具体化。修复产物 = Shield（RP-01 第 1 行唯一 integration 写者）在 Human 批准的 phase0-scoped baseline-repair 派发下，按 `phase0.blocked.detail` 枚举清单在既有 `tests/integration/test_evidence_reuse.py` 内精确创建缺失 planned node id 的测试函数——这是 Shield 在 M-TEST 之外唯一的 tests/integration 写入窗口，scope 仅限该清单，`phase0.sealed` 后关闭（seal 后任何修改按第 5 条漂移 fail-closed）。节点语义验收标准：每个节点（i）落在 blocked detail 枚举的 planned node id；（ii）经公开出口断言对应 v0.6 AC 语义——`test_prism_consumes_runtime_evidence_without_suite_rerun` 断言 PRISM_REVIEW 消费 `red.validated` 证据 identity（selection binding）与隔离 counterexample kill 且期间事件流无普通套件重跑记录（AC-FR0250-03@v0.6），`test_identity_fields_append_only_auditable` 断言 `test.selected` 与执行/复用证据的 selection identity（选择依据+被选节点+baseline/commit+tree_stamp+attempt/actor）append-only 可经 `trac replay`/`trac report` 审计（AC-NFR0130-01@v0.6），`test_stale_propagation_uniform_across_gates` 断言 SELECT_TASK/REFACTOR/SELECT_DIFF/M-VERIFY 复用共用同一 identity 判据且 stale 传播在各门一致可见（AC-NFR0130-02@v0.6）；（iii）被 machine contract collect 收进 inventory 且经 `run_selected` 执行为绿并持久化 evidence。验证方式 = 唯一采信新一次 `phase0_validate` 的机器结果（`phase0.baseline_repaired` 事件携带真实 node id/digest）；Agent 自述与 marker 均不采信。修复不改写历史事件；BLOCKED 经新一次 `phase0_validate` 回 PHASE0_VALIDATING（IF §1c），E-01 示例的 `phase0.baseline_repaired` 输出行即该次 re-validate 的可观察结果。Human 拒绝修复、或修复后节点仍不可 collect/identity 不可恢复 → 停留 BLOCKED，v0.7-A 不成立。
4. Runtime 真跑 registry 的 coverage/guard 命令，要求 coverage ≥95%、source omit 为空、守卫零违规且 parity 通过；D-18 live 目录隔离不是 source coverage omit。
5. `phase0.sealed` 引用 seal blob：v0.6 六文档 digest、全部冻结 test node digest（含三修复节点）、marks/env/adapter 验证结果。后续角色 scope 排除 v0.6 文档与冻结 integration/e2e；任何漂移 fail-closed。
...
| 7 | Phase 0 baseline-repair（BLOCKED 期缺失节点创建，§1.0.2 第 3 条） | ❌ | ✅ Human 批准派发，scope 仅 blocked detail 清单 | ❌ | 评审 | 派发/机器验证独家 |
...
trac run -> cli.main.cmd_run -> Executor.run_loop -> kernel.machine.decide
  ...
  -> [planned nodes absent] phase0.blocked(node_missing, detail=planned ids)
       -> BLOCKED parks run awaiting Human
       -> Human approves Shield baseline-repair dispatch (§1.0.2 item 3)
       -> nodes land in tests/integration/test_evidence_reuse.py
       -> new phase0_validate re-enters PHASE0_VALIDATING (IF §1c)
...
- **FR-0256** owner=executor/phase0.py:AC-FR0256-01 surface=trac-run-Phase0+trac-replay/report composition=phase0_validate→scan_trace_gaps→blocked→Human-approved-Shield-baseline-repair→new-phase0_validate→adapter-run_selected-binding wiring=M-DESIGN-EXIT→M-TEST-entry-sees-unsealed→gap-scan-planned-nodes-absent→phase0.blocked(node_missing,detail=3-planned-ids)→Shield-creates-3-nodes-in-test_evidence_reuse.py→re-validate→three-v0.6-ACs-run_selected→phase0.baseline_repaired test=integration:tests/integration/test_phase0_binding.py::test_gap_acs_bound_to_real_collected_nodes evidence=`.venv/bin/python -m pytest -q tests/integration/test_phase0_binding.py`输出`passed`且事件序含`phase0.blocked(reason=node_missing)`及其后`phase0.baseline_repaired(ac=AC-FR0250-03@v0.6,bound_node.node_id=tests/integration/test_evidence_reuse.py::test_prism_consumes_runtime_evidence_without_suite_rerun)`与AC-NFR0130-01/02两条同构repaired事件 IF-PHASE-001
...
> **Prism [RESOLVED]:** PRISM-ARCH007-R4-01 [blocker|判据7 可实现性 + 判据8 合同真实性 + 判据2 六元组闭合]：AC-FR0256-01 的 Phase 0 修复路径按当前合同不可达成，且因 v0.7 extension 已随回滚残留实现于 HEAD，下次 M-DESIGN EXIT 将首次真实执行 phase0_validate，本矛盾已从潜在变为现实死点。事实：(1) 本行 evidence 断言 events 含 phase0.baseline_repaired(ac=AC-FR0250-03@v0.6, bound_node.node_id=tests/integration/test_evidence_reuse.py::test_prism_consumes_runtime_evidence_without_suite_rerun)，但该函数从未存在于 test_evidence_reuse.py（git log --all -S 对该文件全历史为空；文件现仅 4 个其它函数）；AC-NFR0130-01/02 声明的 test_identity_fields_append_only_auditable、test_stale_propagation_uniform_across_gates 同样全历史不存在——三条 v0.6 缺口 outlet 是 v0.6 test-plan §8 声明而从未落盘的节点（这正是缺口成因）。(2) 设计自身语义把此态判为 BLOCKED 而非 repaired：IF §1d '计划 node 不在 actual collect inventory 时不得生成 repaired'、ARCH §1.0.2 第 2 条 '节点缺失→phase0.blocked，后续 M-TEST 不派发'；真实 phase0_validate 按 scan_trace_gaps 语义必然产出 node_missing→BLOCKED 停摆，本行断言的事件永不出现。(3) 三节点创建无任何已设计通道：Devon scope 排除 tests/integration（layout 合同+§1.0.5 mutation scope ban）；Shield 的 tests/integration 写权限只在 M-TEST 开放，而 Phase 0 门位于 M-DESIGN EXIT→M-TEST 入口之前——先有门、后有唯一测试写入通道，结构性鸡生蛋。(4) 唯一脱困路径 IF §1c 'BLOCKED 只经 Human 修复仓库事实后新一次 phase0_validate' 是未设计通道：谁写三节点、按什么验收标准写、写完如何验证确实覆盖三 AC 语义，三件套零规定；spec.md FR-0256 示例同引该 node id。后果：AC-FR0256-01（integration+e2e required）evidence 不可达成 → 无 phase0.sealed → M-TEST/M-IMPL 全不派发 → v0.7-A happy path 整体不可达（§11.1 主旅程同理）。修复（任选其一，并保持 §1.0.2/IF §1d/§1.2 本行/test-plan §11.1 与 acceptance AC-FR0256-01 相互一致）：(a) 设计显式 pre-gate 基线补齐交付物——把三缺口节点创建指定给时序可行的通道（如 Human 批准的 Shield pre-Phase0 baseline-repair 任务，layout/角色合同相应增补）并给出节点语义验收标准；(b) 定义缺口 re-binding 规则——gap AC 按何种准则绑定到已存在真实节点（准则、选择者、覆盖判定），并同步重写本行 evidence 与 spec 示例（spec 变更走 doc-gap 上游通道）；(c) 把三缺口设计为显式 BLOCKED→Human 修复→re-validate 编排——AC-FR0256-01 的 evidence/验收措辞改为该编排的可观察结果，补全 Human 修复通道合同（修复产物、验证方式），§1.0.2 第 1 条措辞同步。
>> **Archer:** 采用方案 (c)：三缺口设计为显式 BLOCKED→Human 修复→re-validate 编排，修复通道已完整合同化，四处一致性（§1.0.2/IF §1d/§1.2 本行/test-plan §11.1）同步修订。(1) §1.0.2 重写：第 1 条把 planned_bindings 唯一来源钉死为 v0.6 test-plan §8 绑定行并固定三个 node id，明确三节点从未落盘（已复核 git log --all -S 对 test_evidence_reuse.py 全历史为 0 commits）是首次 phase0_validate 的预期输入而非异常；第 2 条 blocked 事件 reason/detail 合同化；新增第 3 条修复通道合同——修复产物=Shield（RP-01 第 1 行唯一 integration 写者）在 Human 批准的 phase0-scoped baseline-repair 派发下按 phase0.blocked.detail 清单在既有 test_evidence_reuse.py 精确创建三个 planned node（这是 Shield 在 M-TEST 之外唯一 tests/integration 写入窗口，seal 后关闭），节点语义验收标准逐条锚定 v0.6 acceptance 原文（AC-FR0250-03=PRISM_REVIEW 消费 red.validated identity+隔离 kill 且无套件重跑；AC-NFR0130-01=selection identity 五元组 append-only 可 replay；AC-NFR0130-02=SELECT_TASK/REFACTOR/SELECT_DIFF/M-VERIFY 同一 identity 判据+stale 一致传播），验证方式=唯一采信新一次 phase0_validate 机器绑定事件，Agent 自述与 marker 不采信。鸡生蛋问题由此消解：门先动（首次校验落 blocked，不静默），节点创建经 Human 批准派发的 Shield 窗口（时序在 BLOCKED park 内，先有门、后有合法写入通道），再经新一次 validate 机器验证——与 SM-01.3/SM-01.5 和 IF §1c 完全自洽，不新增 canonical stage、不改 spec/acceptance 一字（E-01 示例即 re-validate 那次 run 的输出）。(2) interfaces：§1a 规定 node_missing/uncollectable 时 detail 必须 ;-连接枚举缺失 planned id 且 recoverable=true；§1c 把 Human 修复仓库事实具体化为该通道；§1d 补 planned_bindings 来源与落盘后重扫路径（仍必须经 run_selected，无直接 repaired）；IF-PHASE-001 合同同步。(3) §1.2 AC-FR0256-01 行 evidence 改为编排可观察结果：事件序含 phase0.blocked(reason=node_missing) 及其后三条 baseline_repaired（AC-FR0250-03@v0.6+bound_node 精确 id+AC-NFR0130-01/02 同构）；§1.1 composition root 补 blocked 分支；§1.0.10 增第 7 行角色所有权；§5.2 风险同步（node_missing 首验 blocked 是设计内路径，不可恢复=拒绝修复/修复后仍不可绑定）。(4) test-plan：§1.4 增第 9 条写入窗口守卫、§2.4 固定三 planned id 与 fixture 口径、§5 第 5 条、§9 增第 7 行 BLOCKED→PHASE0_VALIDATING 转移、§11.1 主旅程 phase0 段改为 blocked→repair（fixture 扮演修复落盘、门禁全真实）→re-validate→sealed。§8 锚点与 acceptance 措辞无需改动（AC 本就不断言首验无 blocked）。三份 trac validate 均 valid。
>>> **Prism:** 复核通过：方案 (c) 已完整落地且四处一致——(1) §1.0.2 第 1 条钉死 planned_bindings 唯一来源=v0.6 test-plan §8 绑定行并固定三 node id，三节点从未落盘（git log --all -S 全历史 0 commits，本次独立复现实测一致）被定义为首验预期输入而非异常；(2) 第 2 条 blocked reason/detail 合同化（IF §1a：node_missing/uncollectable 时 detail 必须机器可解析枚举缺失 planned id 且 recoverable=true）；(3) 第 3 条修复通道合同完整——修复者=Shield（RP-01 第 1 行唯一 integration 写者）、批准=Human、scope 仅 blocked detail 清单、窗口 seal 后关闭、节点语义验收标准逐条锚定 v0.6 acceptance 原文、验证唯一采信新一次 phase0_validate 机器绑定事件，鸡生蛋死点消除（门先落 blocked 不静默→BLOCKED park 内合法写入窗口→机器 re-validate）；(4) §1.2 AC-FR0256-01 行 evidence 改为编排可观察结果（blocked(node_missing)→三条 baseline_repaired），不再断言不可达成事件；§1.1 blocked 分支、§1.0.10 第 7 行、§5.2 风险、IF §1c/§1d/IF-PHASE-001、test-plan §1.4.9/§2.4/§5.5/§9.7/§11.1 全部同步。与锁定 acceptance 自洽：AC-FR0256-03 要求节点缺失→BLOCKED、AC-FR0256-01 要求 repaired 事件最终出现、spec FR-0256 保留修复→再校验循环（不设预算上限），本编排是三者唯一 reconciliation。三份 trac validate 均 valid。本线程 resolved。
...
config_digest = "sha256:5ccdc0ea96411dce6bb6927889e2e9c6f3f49f93d11b8b585d59e6d5934b293b"
...
- **Phase 0 真实绿基线不可恢复**：三缺口节点在仓库从未落盘，首次校验按 §1.0.2 第 2/3 条进入 BLOCKED→baseline-repair→re-validate 编排——这是设计内路径而非死点；不可恢复指 Human 拒绝修复、修复后节点仍不可 collect/identity 不可恢复、coverage 无法达到或硬守卫无法修复，此时停留 BLOCKED（SM-01.5），不能由设计放宽。
