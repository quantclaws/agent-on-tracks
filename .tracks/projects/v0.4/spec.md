...
15. EXIT -> WRITE：trace 闭合复跑失败（`trac check trace` 检出无测试绑定的 required AC、无主 marker 或短格式 marker），携带 trace findings 重派 Shield 修复 marker/绑定，复用 <=3 升级预算（第 3 次升级 `awaiting_human`/escalation）
