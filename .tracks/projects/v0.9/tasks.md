...
## T-002
- Issue: #142
- Description: 项目登记与就绪探测（IF-PROJ-001）：readiness.py 四类封闭探针逐项 ok+reason；未就绪阻断 create_run；登记归属展示经 T-001（deps 覆盖 service）。锚点=登记一件+就绪两件（0289-02 越范围拒绝需 service.py 新代码而 T-001 已封闭，deferred 至 T-INT 接 guard 后收口）。
