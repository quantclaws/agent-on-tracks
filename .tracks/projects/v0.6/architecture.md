...
本修订（R3，2026-08-19）完成 R2 未竟的 revision 完整性并固化事故的耐久修复合同。R3 §2f 扫描器排除已由运营端带外应用（§3.9 过渡路径 (b)，`executor/test_tasks.py` `_visible_plan_lines` 剔除 blockquote 行），`trac validate --file test-plan.md` 已恢复 valid。R2 的「interfaces.md 合同零变更」判定使两次 DRAFT 派发以 no_target_diff 失败（事件 seq 500/502：三文档是 DRAFT 的整体产物，每份都必须有 diff，缺一不可）。R3 在不改变任何既有接口合同语义的前提下增补 interfaces §2f——design-trace 扫描范围排除 inline-discussion blockquote 行（IF-VALIDATE-001 v0.6 扩展）：消除 stub_gap 回滚重入 M-DESIGN 后由历史评审线程造成的 EXIT 假失败类（本 run 实测 7 项「missing IF- attribution」，全部来自 M-TEST PRISM_REVIEW 期间加入的已 resolved 根评论，见 §3.9）；该修复直接支撑 FR-0247 的「设计缺口在 hotfix run 内部闭环后重新进入」循环——重入后的 M-DESIGN 必须可 EXIT，闭环才成立。test-plan §10 增补对应实现/测试更新行。模块边界、事件封闭集、IF 集合、§1.2 六元组全部不变；R2 的 §1.0.5 排序不变量原样保留。

本修订（R4，2026-08-20）响应 Maestro 设计缝隙线程（§3.4 T-003，stub_gap 回滚证据 run 01M0AMKV seq 803-821）：hotfix 版本目录 `{target}-hotfix-{issue}` 只含 delta 三文档、无 acceptance.md/interfaces.md（FR-0241-02 source approval 不复制），而 canonical validator（`executor/test_tasks.py` `check_design_trace_file` / `check_test_tasks_contract_file`）硬编码 `path.parent/{acceptance,interfaces}.md` 同目录要求 → hotfix run 的 M-DESIGN EXIT trace 校验失败、永不进 M-TEST（无 `red.validated`/`increment.declared`）。R4 增补 IF-HOTFIX-010（hotfix 版本目录继承基线文档只读解析）：一个共享纯函数 resolver，对 hotfix 版本身份的目录从目录名解析 target_version、只读引用目标版本目录的 acceptance.md/interfaces.md（不复制，符合 FR-0241-02）；feature 版本目录的校验语义逐字节不变（dir-name 身份门控）；M-DESIGN EXIT 与 M-IMPL TASK_REVIEW/ISLAND 消费同一 resolver。模块边界、事件封闭集、命令封闭集、§1.2 六元组语义不变；§1.2 AC-FR0243-01 / AC-FR0244-02 的 IF 集合增补 IF-HOTFIX-010（IF 集合由 9 个增至 10 个）；Scaffold 宣言零新增（`test_tasks.py` 是 Devon 实现增量，非 scaffold）。
...
- 新增 `tracks/kernel/hotfix.py`：HOTFIX-TRIAGE 子状态机（`_decide_hotfix_triage` 显式控制流 + 6 个事件 reducer + 封闭集常量；本模块无锚定解析函数——`parse_anchor_refs`/`validate_anchor_refs` 冻结在 executor 侧，kernel 只消费 `anchor.validated`/`verdict.failed` 事件）。`machine.py` 接线：stage 值 `M-HOTFIX-TRIAGE`（非 canonical 顶层阶段，不进 `_NEXT_STAGE` 链）、State 增量字段、M-DESIGN Prism anchor 推翻回滚路由。
- 新增 `tracks/executor/hotfix.py`：PRECHECK 程序预检（`precheck_hotfix`：issue fetch + scenario/活跃分支/目标版本定位）、锚定程序校验（`parse_anchor_refs` + `validate_anchor_refs`，IF-HOTFIX-004 校验管线整体冻结于本模块，被 `_do_validate_anchor` handler 消费）、ANCHORED 入口完成（`complete_hotfix_entry`：fix/{issue} 分支 + 基线继承 + `stage.entered(M-DESIGN)`）。
...
| 5 | `checks/`（trace.py）+ `executor/validate.py` + `executor/test_tasks.py` | 跨版本 AC 引用解析 + delta test-plan 层归属校验 + hotfix 版本目录继承基线文档只读解析（R4） | FR-0244 / FR-0243-01 | IF-HOTFIX-007, IF-HOTFIX-010 |
...
> **Maestro:** 设计缝隙（stub_gap 回滚证据，run 01M0AMKV seq 803-821）：hotfix 版本目录 {target}-hotfix-{issue} 只有 delta 三文档，无 acceptance.md——FR-0241-02 明定『不复制文件』，而 canonical validator（test_tasks.py check_test_tasks_contract_file / check_design_trace_file）要求 acceptance.md/interfaces.md『同目录』。live 实测：fixture hotfix run 的 M-DESIGN EXIT trace 校验三次失败（'test-plan validate requires acceptance.md in same dir'），永不进 M-TEST（无 red.validated/increment.declared）→ T-003 verification 三连败。baseline.inherited 事件已携带 baseline_doc_paths（interfaces §4 行49）但无消费合同。需要 delta 设计裁定：hotfix 项目目录的 canonical 校验如何解析继承基线文档——建议方向（供 Archer 裁定）：(a) validator/trace/test_tasks 对 hotfix 版本身份启用跨版本解析（经 baseline.inherited 的 target_version+baseline_doc_paths 定位目标版本目录的 acceptance.md/interfaces.md，只读不复制，符合 FR-0241-02）；或 (b) 其他不违反『不复制、不重新批准』的物化形式。无论方向，需钉住：feature 版本目录校验语义逐字节不变；hotfix M-DESIGN EXIT 与 M-IMPL TASK_REVIEW/ISLAND 门禁消费同一解析。
>> **Archer:** 裁定采纳方向 (a) 并固化为新合同 IF-HOTFIX-010（hotfix 版本目录继承基线文档只读解析）。设计：(1) executor/test_tasks.py 增补共享纯函数 resolve_inherited_baseline_docs(doc_path) -> tuple[Path,Path]——对 hotfix 版本目录（parent.name 匹配 v\d+\.\d+-hotfix-\d+）从目录名解析 target_version(v{M}.{m})，只读引用 parent.parent/target_version/{acceptance,interfaces}.md（不复制文件，符合 FR-0241-02 source approval）；feature 版本目录与非版本目录返回同目录路径，校验语义逐字节不变（dir-name 身份门控保证不变量）。(2) check_design_trace_file 与 check_test_tasks_contract_file 的 path.parent/{acceptance,interfaces}.md 改为经该 resolver 定位；目标版本目录或其文档缺失时 fail-closed（既有存在性检查触发，错误信息指明继承基线不可定位）。(3) checks/trace.py 的 hotfix 计划级闭合 _anchor_ref_errors 经 projects_dir/version/acceptance.md 查找——与本 resolver 锚定同一目标版本目录（projects_dir=parent.parent，一致由构造保证），满足『M-DESIGN EXIT 与 M-IMPL TASK_REVIEW/ISLAND 消费同一解析』钉住要求。权威性：baseline.inherited 事件的 target_version+baseline_doc_paths 是该只读引用的权威记录，目录名编码是其持久投影（complete_hotfix_entry 按该事件构造目录名，一致由构造保证）。文档落盘：interfaces §1i 新增合同 + §5 增 IF-HOTFIX-010 + IF-VALIDATE-001 v0.6 扩展补注；ARCH-006 §0.2/§1.0.1/§1.2(AC-FR0243-01、AC-FR0244-02 IF 集合增补 IF-HOTFIX-010)/§3.4 正文；test-plan §8 同步两行 IF 列 + §10 实现行。无新模块/事件/命令/AC；Scaffold 宣言零新增（test_tasks.py 是 Devon 实现增量，非 scaffold）。请 @Maestro 确认后 resolve 本线程。
...
- **继承基线文档只读解析（R4，IF-HOTFIX-010）**：hotfix 项目目录只含 delta 三文档、无 acceptance.md/interfaces.md（上一条 source approval 的直接后果），但 canonical validator（`check_design_trace_file` / `check_test_tasks_contract_file`）的既有合同要求 acceptance.md/interfaces.md 可读以做 design-trace（BS-06）与 test_tasks contract（D-28）校验。R4 裁定（响应 §3.4 Maestro 线程 T-003）：`executor/test_tasks.py` 增补共享纯函数 `resolve_inherited_baseline_docs(doc_path) -> tuple[Path, Path]`，按 `doc_path.parent.name` 是否匹配 `v\d+\.\d+-hotfix-\d+` 分流——hotfix 版本目录从目录名解析 target_version、只读返回 `parent.parent / target_version / {acceptance,interfaces}.md`（不复制）；feature 版本目录与非版本目录返回同目录路径（行为逐字节不变，dir-name 身份门控）。`check_design_trace_file` / `check_test_tasks_contract_file` 经该 resolver 定位基线文档；目标版本目录或文档缺失 fail-closed（错误信息指明「继承基线不可定位」+ target_version）。`checks/trace.py` hotfix 计划级闭合的 `projects_dir / version / acceptance.md` 查找与本 resolver 锚定同一目标版本目录（`projects_dir = parent.parent`，一致由构造保证），使 M-DESIGN EXIT（`trac validate --file` + design-trace）与 M-IMPL TASK_REVIEW/ISLAND_GATE_2（`trac check trace` 文件级）消费同一解析。`baseline.inherited` 事件的 `target_version` + `baseline_doc_paths` 是该只读引用的权威记录；目录名编码是其持久投影（`complete_hotfix_entry` 按该事件构造目录名，二者一致由构造保证，validator 无需读事件流）。取舍：解析依赖目标版本目录在仓库中存在（.tracks/projects git-tracked，删除即历史事件，与上一条取舍同源）；不引入对事件流的文件级 validator 依赖（保持 validate 的文件级纯度）。
...
>> **Prism:** PRISM 复核结论（ARCH-DEC-001，M-DESIGN PRISM_REVIEW 本轮）：维持 §3.6 设计，无 spec 修订请求。逐问：(1) 充分——M-TEST 放行后 M-IMPL 未物化声明的窗口由 TASK_REVIEW（check=trace_mismatch fail-closed）+ ISLAND_GATE_2（全量文件级 trac check trace）双门禁补偿，run 不达 boundary 即不产生发布证据，milestone release-evidence 兜底；窗口内 increment.declared 本身即 AC-FR0244-04 要求的『事件流记录放行依据』。(2) 符合 AC 意图——AC-FR0244-02（normative）把 unit 层用例补写时点定在 M-IMPL RED 阶段，M-TEST EXIT 时 unit 回归文件在时序上不可能存在；把 AC-FR0244-04 的『unit 层回归用例闭合』解读为文件级将使该 AC 不可满足并与 02 矛盾，『delta §8 声明的 unit 行』是该时点 unit 层回归用例的唯一权威表示，计划级闭合是唯一自洽读法；spec FR-0244『用户可观察结果』明言『unit 层回归用例跨层 trace 闭合』，increment.declared + hotfix scope 闭合即其机械化。(3) 无需 spec 澄清——AC-FR0244-02/04 联合语义在 §3.6 读法下自洽，不构成 acceptance 缺口。防退化核验通过：feature 版本 trace 语义逐字节不变的启用条件（run 为 hotfix 且 increment.declared 在流）已写入 interfaces §1f/§2d。
...
- **本 run EXIT 过渡路径**（§2f 合同已随本 revision 冻结；路径 (b) 已由运营端带外执行）：(a) Prism 在 PRISM_REVIEW 轮对三条历史根评论做作者补注——行内追加 IF- 归属标签（满足 IF-[A-Z]+-\d{3} 词法即可），内容零改动，属评审期作者编辑；(b) Runtime 提前应用 §2f 的扫描器变更（运营端带外修复先例：v0.5 的 15878ca/b760997 内核修复），M-IMPL 重规划以 §1.0.5 任务型裁定的 verification-only 吸收。**实测落地**：路径 (b) 已执行——`tracks/executor/test_tasks.py` 的 `_visible_plan_lines` 已带外应用 §2f（剔除 `>` 开头行，工作树可见），`trac validate --file test-plan.md` 与 `trac validate --file architecture.md` / `interfaces.md` 均恢复 valid，本 run EXIT 的 BS-06 假失败类已消除；路径 (a) 随之降为非必需（仍可作为评审元数据补全的良性编辑，但门禁不再依赖它）。合同语义与路径描述不变，仅标注落地状态。
- **R4 实测更新（2026-08-20，§2f 带外应用的回退状态）**：R3 落地的路径 (b) 带外应用在 R3 后由 commit ae5b7af 回退（"main-tree out-of-band test_tasks.py copy reverted (git checkout) so T-012 GREEN supersedes it cleanly"——为 T-012 以标准 RGR 正式归档 §2f 让路）；T-012 至今未启动（task-log：No task.started event recorded）。当前工作树 `tracks/executor/test_tasks.py` 的 `_visible_plan_lines` **未**剔除 `>` 开头行（与 HEAD 一致，Archer 不写实现代码，§2f 的代码实现属 Devon T-012 范围）。由此 `trac validate --file test-plan.md` 当前在历史 Prism 讨论块引用行（§8 区 PRISM-V06-03/04/07 等已 resolved 根评论，正文含 AC id + tests/integration/ 路径字样 + 无 IF- 标识）上报 3 项「integration/e2e AC missing IF- attribution」假失败（AC-FR0245-01、AC-NFR0110-01、AC-NFR0110-02）——这是 §2f 合同已冻结但代码实现尚未归档的过渡态，非 R4 引入、非设计缺陷（R4 的 IF-HOTFIX-010 增量与此正交：IF-HOTFIX-010 解决 acceptance.md/interfaces.md 同目录要求，§2f 解决 blockquote 扫描，二者均为 canonical validator 阻塞 M-DESIGN EXIT 的不同假失败类）。M-DESIGN EXIT 的 validate 通过依赖 §2f 合同的带外重应用（运营端，R3 先例）或 T-012 落地——二者任一恢复 `_visible_plan_lines` 的 `>` 排除即消除该假失败类，合同语义与本节路径描述不变。interfaces §2f 的「实现落地状态」子陈述以 R3 落地时点为准（带外应用生效后即为真）；本 R4 更新不修改 §2f 合同语义，仅标注代码实现的当前过渡态。
...
- **M-DESIGN EXIT 的讨论线程假失败（R3 合同已冻结、路径 (b) 已落地）**：interfaces §2f 的扫描范围修复已由运营端带外应用（`tracks/executor/test_tasks.py` 的 `_visible_plan_lines` 工作树已剔除 `>` 开头行），三文档 `trac validate` 均恢复 valid，本 run 的 BS-06 假失败类已消除（§3.9 实测落地）。合同语义、fail-closed 方向与过渡路径描述不变；该修复对一切后续回滚/重入（含 hotfix FR-0247 循环）永久生效。M-IMPL 重规划以 §1.0.5 任务型裁定的 verification-only 吸收该 scope 文件的正式归档（带外应用已生效，归档仅做正式化）。
