...
- **实现状态与归档职责（R5 收敛，2026-08-20）**：§2f 合同随本 revision 冻结（durable：`_visible_plan_lines` 剔除 inline-discussion blockquote 行，layer/IF- 归属判定面收窄为非 blockquote 可见行，fail-closed 方向不变）。代码实现状态（工作树 vs HEAD、T-012 标准 RGR 归档、运营端带外应用）是 Devon/Runtime 的归档职责，非设计合同事项——R3/R4 的带外应用/回退/重应用实测记录已多次随工作树代码变化失真，故本节撤销该代码级工作树快照跟踪。当前事实（截至本 revision）：工作树 `tracks/executor/test_tasks.py` 的 `_visible_plan_lines` 已反映 §2f（剔除 `>` 开头行），三文档 `trac validate` 均为 valid，M-DESIGN EXIT 的 BS-06 讨论线程假失败类已消除；若后续工作树代码偏离 §2f，门禁以「validate 失败」自暴露，属实现回归而非设计缺口，按 §2f 合同由 Devon/Runtime 修复。R4 的 IF-HOTFIX-010 增量（acceptance.md/interfaces.md 同目录要求）与 §2f 正交，二者均为 canonical validator 阻塞 M-DESIGN EXIT 的不同假失败类，均已合同冻结。M-IMPL 重规划按 §1.0.5 batch B/C 的 verification-only 吸收该 scope 文件的正式归档。
...
- **M-DESIGN EXIT 的讨论线程假失败（§2f 合同已冻结）**：interfaces §2f 的扫描范围合同冻结——inline-discussion blockquote 行不参与 layer/IF- 归属扫描。当前工作树 `tracks/executor/test_tasks.py` 的 `_visible_plan_lines` 已反映 §2f，三文档 `trac validate` 均为 valid，BS-06 假失败类已消除（§3.9 R5 收敛）。代码实现级偏离由 validate 失败自暴露（实现回归，非设计缺口），按 §2f 合同由 Devon/Runtime 修复；合同语义对一切后续回滚/重入（含 hotfix FR-0247 循环）生效。M-IMPL 重规划以 §1.0.5 任务型裁定的 verification-only 吸收该 scope 文件的正式归档。
