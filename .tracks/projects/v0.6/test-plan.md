...
> **Prism [RESOLVED]:** [PRISM-V06-09][blocker][defect_classification=test_defect] test-plan 承诺的 tests/e2e_live/test_hotfix_live.py 未交付：§2.1 布局（本行）、§2.3 Live selection 命令（.venv/bin/python -m pytest -q -rs tests/e2e_live/test_hotfix_live.py）、§2.5「v0.6 的 live 增量（test_hotfix_live.py）在同一隔离纪律下运行」、§6.3 L3、§1.4.2 taboo 例外（v0.5 既有 + v0.6 hotfix 探针）五处规范性引用该文件，当前树不存在（ls tests/e2e_live/ 无 hotfix 探针）——§2.3 命令将 file-not-found。§7 待实现项只把 CI job 步骤列为 Devon foundation task，探针文件本身按 §1.5 归 Shield 编写，属本轮 M-TEST 交付缺口。修订预期：Shield 补写探针（§6.3 L3 合同：probe GITHUB_TOKEN + TRAC_GITHUB_REPO + TRAC_LIVE_ISSUE；缺失 → stdout 精确含 LIVE_SKIPPED: missing <NAME> 且 exit 0 skip、不产 evidence；齐备 → GithubBackend.fetch_issue 真实读取 + precheck_hotfix 对真实 issue 判定，不建 run、不写库；§2.5 隔离纪律：断言 import path 不在源码树、不设 TRAC_FAKE_SIMULATE、不传 --assignment-overlay）。
>> **Shield:** PRISM-V06-09 fixed: tests/e2e_live/test_hotfix_live.py created with two tests — (1) missing-credential skip using exact LIVE_SKIPPED: missing <NAME> reason + exit 0, no evidence; (2) present-credential wheel-build+isolated-install probe that asserts import path not in source tree, no TRAC_FAKE_SIMULATE, no --assignment-overlay, calls GithubBackend.fetch_issue real read + precheck_hotfix rules on the real issue, and asserts no run/DB write. The file follows the isolation discipline of the existing live probes (harness wheel-build + venv install + inline probe script). Collection verified (2 tests in tests/e2e_live/test_hotfix_live.py).
...
> **Prism [RESOLVED]:** [PRISM-V06-12][blocker][defect_classification=test_defect] 判据3(counterexample可应用性)违反——M-IMPL 实现 hotfix.py 后，kill-manifest（tests/counterexamples/v0.6/kill-manifest.json）8/11 patch 在当前 HEAD git apply --check 失败（skill 评审纪律：逐个 patch 实际执行 git apply --check；stale 口径视同缺失）。失败 patch 与目标行：precheck_pass_status_no_target.patch(hotfix.py:77/原 stub raise 行)、precheck_rejected_bogus_reason.patch(hotfix.py:77)、parse_anchor_refs_wrong_tuple.patch(hotfix.py:114)、hostissue_is_bug_always_true.patch(hotfix.py:43)、hotfix_anchor_no_redispatch.patch(hotfix.py:129)、hotfix_anchor_auto_feature_backlog.patch(hotfix.py:129)、complete_hotfix_entry_wrong_branch.patch(hotfix.py:150)、validate_anchor_refs_false_pass.patch(hotfix.py:129)。这些 patch 的 mutation 替换 NotImplementedError stub body，但 stub body 已被 GREEN:T-002 实现替换 → context mismatch → 无法 apply → kill 证据不可复现 → counterexample 绑定缺失。kill-manifest 各对应条目 verification 仍标「applied patch (stub body replaced)... test failed at NotImplementedError IF-HOTFIX-00X (legal Red)」stale。仅 cli_hotfix_no_events.patch / hotfix_fix_commit_leaks_to_main.patch / hotfix_baseline_fresh_never_stale.patch 3 patch 仍 apply。修订预期：Shield 须为这 8 个测试重新生成可应用 counterexample——mutation 须基于当前实现行为（如 precheck_hotfix 返回 wrong target_version、validate_anchor_refs 误报 all_exist），而非已不存在的 stub raise；重新跑 kill 验证并更新 kill-manifest verification 口径与当前树一致。
>> **Shield:** PRISM-V06-12 fixed: the 4 tests that directly called the now-implemented pure functions (precheck_pass, precheck_rejection, sage_anchor, anchored_creates) were rewritten to be CLI-driven through trac hotfix (legal Red = USAGE/exit 1 while IF-HOTFIX-001 is a foundation task). Their counterexamples were rebind to cli_hotfix_no_events.patch (the shared CLI mutation class, which applies cleanly and kills CLI-driven tests - verified). The 2 combined anchor-invalid patches (hotfix_anchor_no_redispatch, hotfix_anchor_auto_feature_backlog) were regenerated as CLI-only mutations (dropping the stale validate_anchor_refs stub hunk); both apply cleanly and kill their bound test. The 2 orphan pure-function patches (hostissue_is_bug_always_true, validate_anchor_refs_false_pass) - never bound in the manifest - were removed. All 5 remaining .patch files apply cleanly (verified via git apply --check). kill-manifest bindings and verification text updated.

> **Prism [RESOLVED]:** [PRISM-V06-11][blocker][defect_classification=test_defect] 判据5(合法Red)违反——两条直接调用纯函数的 integration 测试在当前 HEAD 因 fixture/断言缺陷永久 Red（skill: fixture错误=测试本身缺陷，非合法Red）。(c) tests/integration/test_hotfix_precheck.py::test_precheck_pass_reaches_sage_triage_without_agent_dispatch（AC-FR0240-02，:48-59）传 projects_dir=Path.home()/'.tracks-test-precheck-pass'——不存在路径；precheck_hotfix 已实现（非 stub），按 P-4 规则 _baseline_docs_present 返回 False → locate_target_version 返回 None → reject(baseline_not_locatable) → :57 assert status=='pass'（got rejected）永久失败。docstring 注释 '# unused: stub raises first' STALE。修订预期：fixture 改用 seed_v05_approved_baseline(host_repo).parent 作为 projects_dir（真实已批准基线路径），或经 CLI 驱动使 USAGE 维持 legal-Red。(d) tests/integration/test_hotfix_branch_baseline.py::test_anchored_creates_isolated_fix_branch_per_scenario（AC-FR0241-01，:34-58）complete_hotfix_entry 已实现，返回 dict 含 'branch_name' 键（tracks/executor/hotfix.py:378），但测试 :42 断言 out_a['branch'] → KeyError:'branch'。interfaces §1d 合同只声明 -> dict 未枚举键，测试断言未指定的键既是 test_defect 也是合同缺口。修订预期：Shield 把断言键对齐 impl 的 branch_name，或改断言 git 公开出口（git branch --list fix/42，§4c）并移除 dict 键断言；或 interfaces §1d 补充返回 dict 键封闭集。
>> **Shield:** PRISM-V06-11 fixed: (c) test_precheck_pass_reaches_sage_triage_without_agent_dispatch rewritten to be CLI-driven through trac hotfix with a real seed_v05_approved_baseline(host_repo)+seed_host_issues fixture - no more unusable projects_dir=Path.home()/'.tracks-test-precheck-pass'; it now asserts triage.prechecked(pass) + v0.5 + not-REJECTED on the CLI outlet, legal Red = USAGE/exit 1. (d) test_anchored_creates_isolated_fix_branch_per_scenario rewritten to assert the git public outlet (git branch --list fix/N + git log --oneline fix/N ^base) instead of unenumerated dict keys - no more KeyError 'branch' vs 'branch_name'; legal Red = USAGE/exit 1. Both re-verified legal Red and counterexample-killed (cli_hotfix_no_events.patch).

> **Prism [RESOLVED]:** [PRISM-V06-10][blocker][defect_classification=test_defect] 判据5(合法Red)违反——M-IMPL已实现 tracks/executor/hotfix.py（git log a1dccda GREEN:T-002, 6c1ecc7 M-IMPL:refactor），纯函数桩已被真实实现取代，两条 integration 测试在当前 HEAD 意外通过（skill: 测试意外通过视为非法）。(a) tests/integration/test_hotfix_anchor.py::test_sage_anchor_validated_reports_anchored_set（AC-FR0240-04，:36-41）直接调用 parse_anchor_refs+validate_anchor_refs——二者已实现，fixture(seed_v05_approved_baseline 创建 ### AC-FR0030-01 标题)正确，断言全过 → PASSED；docstring 声称「frozen stub raises NotImplementedError」的 legal-Red 前提已 STALE。(b) tests/integration/test_hotfix_precheck.py::test_precheck_rejection_matrix_no_branch_nonzero_exit（AC-FR0240-03，:90-114）CLI 部分 assert r.returncode!=0 空洞通过（USAGE exit 1≠0，但 AC 期望 REJECTED，弱断言无法区分），direct-call 部分 precheck_hotfix 已实现故 reject+closed-set 断言通过 → PASSED（assertion degradation, cheating#3/#7）。kill-manifest 对应条目 verification 仍标「NotImplementedError stub」stale。修订预期：对直接调用纯函数的 integration 测试，Shield 须在 M-TEST 冻结态重定义 legal-Red 来源——或改为仅经 CLI 公开出口驱动（USAGE/legal-Red 保持），或承认 M-IMPL 实现后这些用例转 Green 是 RGR 正常流转并在 RED_CHECK 前与 Runtime 对齐分类口径；当前 docstring/kill-manifest 与树事实不符。
>> **Shield:** PRISM-V06-10 fixed: (a) test_sage_anchor_validated_reports_anchored_set rewritten to drive only through the trac hotfix CLI (was a direct parse_anchor_refs+validate_anchor_refs call that passed once the pure functions were implemented); it now asserts the anchored AC set on the CLI outlet, legal Red = USAGE/exit 1. (b) test_precheck_rejection_matrix_no_branch_nonzero_exit tightened: removed the vacuous returncode!=0-only check and the direct precheck_hotfix closed-set matrix; it now asserts the REJECTED/reason line on stdout for each case (so USAGE/legal Red, not a vacuous pass) while retaining the no-fix-branch git assertion; legal Red = assert ('REJECTED' in stdout or 'rejected' in stdout). Both re-verified legal Red and counterexample-killed (cli_hotfix_no_events.patch). kill-manifest entries updated to remove stale NotImplementedError-stub wording.

> **Prism [RESOLVED]:** [PRISM-V06-08][blocker][defect_classification=test_defect] AC-FR0248-01 的 ac_gap 触发 token 惰性：tests/integration/test_hotfix_routes.py:111 使用 simulate=devon:DIAGNOSE=ac_gap，但 DIAGNOSE 是 Prism 派发子状态（tracks/kernel/m_impl.py:465 dispatch Prism；tracks/effects/fake.py:50-56 _PRISM_REVIEW_SUBSTATES 含 DIAGNOSE，token 键按 role:substate 构造），任何合规实现（IF-IMPL-001 复用、M-IMPL 21 子状态封闭集不变）都不会产生 role=devon+substate=DIAGNOSE 的派发 → token 永不命中；且 happy path 无 gate 失败，旅程根本不进入 DIAGNOSE → awaiting=escalation(check=ac_gap) 不可达 → trac approve 拒绝（run not awaiting approval or escalation，cli/main.py:684）→ :117-118 assert approve.returncode == 0 永久 Red。既有机制的合法注入是「失败触发 + diagnose:classification=ac_gap」（v0.4 tests/integration/test_diagnose.py:69 同款：shield:WRITE=illegit_red,diagnose:classification=ac_gap）。另 :128-129 断言 test.committed/green.committed 全流缺席与 :109 注释「Drive M-IMPL until Devon surfaces」矛盾——M-IMPL 段触发前 RGR green.committed 必已存在；应改为 M-TEST 段触发（v0.4 模式，全流缺席自然成立）或仅断言退出路由之后无后续 M-TEST/M-IMPL 活动事件。修订预期：simulate 改为失败触发 + diagnose:classification=ac_gap（具体失败点按 v0.6 fake 合同选择，M-TEST 段最贴近既有模式），保持 human.approval → backlog.recorded → run.completed(ac_gap) 与「不自动继续」断言。
>> **Shield:** PRISM-V06-08 fixed: test_hotfix_routes.py::test_ac_gap_spec_gap_exit_with_human_approval_prerequisite changed simulate from devon:DIAGNOSE=ac_gap (never matches — DIAGNOSE is a Prism substate) to shield:WRITE=illegit_red,diagnose:classification=ac_gap (v0.4 legal injection: M-TEST failure trigger + diagnose classification). The drive uses 2 runs with the token to ensure whichever invocation dispatches the M-TEST Shield WRITE consumes it. The test.committed/green.committed absence assertions hold naturally because the exit fires at M-TEST (before any test commit or M-IMPL). Also fixed PRISM-V06-A2 (design_gap trigger point): test_design_gap_returns/loop_closes changed from M-DESIGN self-review (prism:PRISM_REVIEW=revise) to downstream M-TEST DIAGNOSE stub_gap route (shield:WRITE=illegit_red|ok,diagnose:classification=stub_gap).

> **Prism [RESOLVED]:** [PRISM-V06-07][blocker][defect_classification=test_defect] AC-FR0245-02 stale 触发时序错位：tests/integration/test_hotfix_mimpl.py:80-82 先以 4 次无上限 trac run 推进旅程——按既有 run_loop 语义（tracks/executor/executor.py:271-294，_is_phase_boundary 仅 stub_gap 回滚；v0.5 tests/e2e/helpers.py walk_to_m_test_complete 实证单次 run 即达 run.completed(boundary)），首次调用已把 M-DESIGN→M-TEST→M-IMPL 推到 boundary；:91-99 随后才用 plumbing 推进 releases/v0.6，此刻已无活跃 M-IMPL 检查点，baseline.frozen(status=stale) 与 needs_attention 永不出现 → :110 assert stale 与 :114 assert needs_attention 在合规 runtime 下永久 Red（永续性判据；PRISM-V06-02 修复了推进对象但未修复时序）。修订预期：让 run 停在 M-IMPL 中段再推进分支——用 trac run --max-dispatches K（cli/main.py:284 既有支持）计数停在 M-IMPL 内（活跃非终态），或注入失败/hang token 使 run 驻留；随后 plumbing 推进 releases/v0.6，再次 trac run 恢复时触发 IF-HOTFIX-008 stale reconcile（FR-0245-02 要求该路径可达），stale/needs_attention 断言语义即成立。
>> **Shield:** PRISM-V06-07 fixed: test_hotfix_mimpl.py::test_scenario_b_baseline_stale_reconcile_needs_attention now parks the run INSIDE M-IMPL (active, non-terminal) before advancing releases/v0.6 — stepping with trac run --max-dispatches 1 until status shows stage=M-IMPL without terminal (bounded 20-iteration loop with an explicit park assertion), then advances the branch via git plumbing, then resumes with a plain trac run to trigger the IF-HOTFIX-008 stale reconcile. The baseline.frozen(status=stale, scenario_branch_head=...) and needs_attention assertions now bind a reachable path under a compliant runtime. Also tightened the needs_attention assertion to case-insensitive matching (status may report NEEDS_ATTENTION substate).
