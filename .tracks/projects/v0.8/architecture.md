...
feature：operations.feature 全量（merge main、公开 tag、artifact、release）。post-release hotfix：从 `fix/{issue}` 分支经 M-VERIFY 后 merge main；当远端存在与目标 base version 同名的 `release/{minor}` 分支（活跃 release 分支判定，`when=active_release_branch`）时同步 merge 该分支，并按 `[version] patch_line`（现有 patch 最大值 +1）产生 patch tag/release。**FR-0277-02 分叉同步（严格版）**：条件步目标与 candidate 真分叉时，在 Human 决定前生成并验证本地产物 P（`executor/sync_product.py`：merge-base-aware 三方合并 + 固定身份 commit，父序 B→C；隔离 checkout 上执行声明的 gates/FULL 选择（含 Known Issue waiver）/security/required CI），`{target, B, C, P, product_tree, evidence_digests}` 进入 operation plan/preview_digest；发布仅经 `push_sync_product` 的 `--force-with-lease=<ref>:<B>` 消费已批准 P（tip==P→reconciled_skip、tip==B→执行、其他→conflict/stale），WAL 记录完整身份且恢复只复用同一 P。既有 FF 直通（tip 为 C 祖先）与无活跃分支静默省略不变；`push_merge` 保持 FF-only、不再在发布期生成产品。dev hotfix：precheck 要求活跃 release 分支存在（缺失即 fail-closed 退出非零、不建 fix 分支），仅 merge 活跃分支并打 prerelease_tag（不打公开 tag、不建 release 对象），`trac report` 显示 channel=pre-release。三旅程共享 candidate 绑定与逐操作幂等。
...
#### 1.0.14.1 逃生门 batch 1 交付边界与指针迁移后续任务（b93 裁定）

- **batch 1 交付面收敛**：escape 实现任务（T-004 族）的交付面 = universal return 门禁（M-IMPL NEEDS_ATTENTION 与 DIAGNOSE/escalation 等新源 × canonical 序上游目标，canonical 序唯一来源 `tuple(machine._STAGES) − M-REQ-APPROVAL + release.RELEASE_STAGES`，拒绝零事件）+ writer_lock 内事件序列（`escape.barrier_established`[cutover_seq 取事件库持久最大 seq，barrier 事件 seq=cutover+1，跨进程单调] → `evidence.staled`[仅实际存在桶] → `human.return`[双轨 payload actor/from/to/to_stage]）+ 不可逆 `already_executed` 报告与 `--confirm` 显式确认门（无确认零事件不动指针）+ escape 专属 status 五片段渲染（`human_return=<actor>→<to>`/`evidence.staled=<count>`/`barrier=established`/`late_outcome=quarantined`/`frozen_tests=unfrozen`，函数级 carve，status 行总装与 release 侧片段仍归 release CLI 面任务）。
- **明确不含**（防假绿与越界）：真实指针迁移完成、真实 quiesce 在飞 dispatch 取消与 escape.late_outcome 拦截（payload 契约先行；真实集成已登记为 deferred acceptance item——AC-FR0287-02 的 quiesce 半句，归下方 batch 2 后续任务，由 PLANNING 注册，见 test-plan §8.1）、release 侧 status/report/replay 与 cmd_release/cmd_abandon、reconciled_skip（IF-PUBLISH-002，T-021/T-039 时代锚点）、release 五阶段注册。**Issue #127（quarantine_late_outcome 对缺失/非法 seq fail-open）不属本 run 范围，任何任务不得顺带修改。**
- **指针迁移后续任务（batch 2，依赖 escape 实现任务，函数级 carve 不并入 T-039）**：(1) executor rollback 目标闭合集扩展——现仅 `("M-STORY","M-SPEC","M-ACC","M-DESIGN")`，越集静默 audit blob 无事件；扩展为含 M-TEST/M-IMPL（release 五阶段注册后随 canonical 序自然纳入，闭合集改为序表推导不手写）；(2) `kernel/machine.py` `_on_stage_rolled_back` 按目标 `StageDef.initial_substate` 路由——M-TEST=DISPATCH、M-IMPL=BASELINE（禁固定 DRAFT：M-TEST 无 DRAFT 路由、M-IMPL 初始 BASELINE，固定 DRAFT 会停成非 awaiting 死局且 `trac return` 亦拒，run 砖化）；(3) kernel m_impl RETURNED 路由 reason 镜像 m_test（`"human_return" if s.returned else "diagnose_rollback"`，修审计语义错标）；(4) **真实 quiesce 与 escape.late_outcome 拦截**（deferred acceptance item，AC-FR0287-02 quiesce 半句）：executor dispatch 循环消费 cutover——barrier 建立后在飞 dispatch 取消/冻结、迟到 outcome（`seq ≤ cutover_seq`）落 `escape.late_outcome(quarantined)` 且禁止 checkpoint/publish/覆盖 State。「指针真实迁移完成」断言（return 后 `trac run` 落 `stage.rolled_back`、目标阶段以正确初始 substate 停车、该态可再被回拨）与 quiesce 拦截断言（在飞 dispatch 取消 + 迟到 outcome quarantine 可 replay）均登记到该任务锚点层。
...
- **FR-0277** owner=executor/release_gate.py:AC-FR0277-02 surface=trac-hotfix-post-release+git+trac-replay composition=operations.post_release 条件步 wiring=fix 分支验证→merge main→活跃 release 分支同步 merge→patch tag/release；预验证/M-VERIFY/安全/Human gate 同 feature；真分叉时批准前生成+验证 P（gates/FULL 选择/security/required CI，证据绑定 P）并随 plan/preview/决定绑定 B/C/P/tree/evidence，发布用 expected-old 原子更新且恢复只复用同一 P test=integration+e2e:tests/integration/test_journey_versioning.py::test_post_release_patch+tests/e2e/test_hotfix_release_journeys.py::test_post_release_hotfix_journey evidence=`.venv/bin/python -m pytest -q tests/integration/test_journey_versioning.py`输出`passed`且 main 与 release 分支均含修复、patch tag 存在 IF-JOURNEY-001
...
- **FR-0287** owner=cli/main.py+executor/escape.py:AC-FR0287-01 surface=trac-return+trac-status/replay composition=通用回拨（SM-01.19，既有 return 子命令升级） wiring=M-IMPL 停车（NEEDS_ATTENTION/DIAGNOSE-escalation，失败注入）→trac return --to M-TEST→门禁接受（拒绝零事件）→human.return 双轨 payload(actor,from,to,to_stage,reason)→Runtime 不阻止→advisory 仅记录不改状态→kernel 采纳 RETURNED；batch 1 不含指针迁移完成断言（归 §1.0.14.1 后续任务锚点层） test=integration:tests/integration/test_escape_gate.py::test_universal_return_moves_pointer evidence=`.venv/bin/python -m pytest -q tests/integration/test_escape_gate.py`输出`passed`且 human.return 双轨与 advisory 分离、status 显示 human_return=<actor>→<to> IF-ESCAPE-001
- **FR-0287** owner=executor/escape.py:AC-FR0287-02 surface=trac-replay+trac-status composition=escape barrier/cutover wiring=M-IMPL 停车→trac return --to M-TEST（可达上游，b93 B2）→escape.barrier_established(cutover_seq 取事件库持久最大 seq，跨进程单调)→quiesce 在飞 dispatch（payload 契约）→late outcome(outcome.seq≤cutover_seq) escape.late_outcome(quarantined)→禁止 checkpoint/publish/覆盖 State test=integration:tests/integration/test_escape_gate.py::test_escape_barrier_quarantines_late_outcomes evidence=`.venv/bin/python -m pytest -q tests/integration/test_escape_gate.py`输出`passed`且 late outcome 不产生 design.committed/publish.executed、status 显示 late_outcome=quarantined IF-ESCAPE-001
- **FR-0287** owner=executor/escape.py:AC-FR0287-03 surface=trac-status+trac-replay composition=回拨证据失效 wiring=夹具 seed 下游证据链（candidate.frozen+release 侧最小集，先例 v07_closure_seed）→回拨到 M-TEST→仅实际存在桶 evidence.staled(reason=human_return)→重进不复用；回拨到 M-TEST 前解除测试冻结(frozen_tests=unfrozen) test=integration:tests/integration/test_escape_gate.py::test_return_stales_downstream_evidence evidence=`.venv/bin/python -m pytest -q tests/integration/test_escape_gate.py`输出`passed`且 stale 清单可 replay、不存在桶无 stale 事件、重走无旧证据复用 IF-ESCAPE-001
- **FR-0287** owner=executor/escape.py+executor/publish.py:AC-FR0287-04 surface=trac-return+trac-replay composition=不可逆操作确认与 reconcile wiring=夹具 seed publish.executed(done)→trac return 无 --confirm→already_executed 清单+非零退出+零事件不动指针→带 --confirm→事件序列正常→指针移动；重进 M-PUBLISH 识别 reconciled_skip 的断言归 T-021/T-039 时代锚点（b93 B2 拆分，batch 1 无 M-PUBLISH 路由） test=integration:tests/integration/test_escape_gate.py::test_irreversible_confirm_then_reconcile_skip evidence=`.venv/bin/python -m pytest -q tests/integration/test_escape_gate.py`输出`passed`且确认前零事件、确认后事件序列完整 IF-ESCAPE-001 IF-PUBLISH-002
- **FR-0287** owner=cli/main.py+executor/escape.py:AC-FR0287-05 surface=abandon-CLI+trac-status+git/GitHub composition=轻量终止（SM-01.21/22，新子命令待实现 foundation task） wiring=正常引导（init→start→run，禁裸 trac run）→abandon --reason→run.completed(terminal=cancelled)→零外部副作用(无新 tag/branch、issue 状态不变)→trac run 拒绝提示（_err 走 stderr，断言查 stdout+stderr，b93 B2）、重做经 start 新 run；锚点由 release CLI 面任务（T-001 族）承载 test=integration:tests/integration/test_escape_gate.py::test_abandon_terminal_zero_side_effects evidence=`.venv/bin/python -m pytest -q tests/integration/test_escape_gate.py`输出`passed`且 for-each-ref 无新增、终态 run 推进被拒退出非零 IF-ESCAPE-002
