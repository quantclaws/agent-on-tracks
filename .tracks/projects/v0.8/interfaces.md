...
| 30 | `escape.barrier_established` | `cutover_seq: int`（**取自该 run 事件库持久最大 seq**，barrier 事件自身 seq = cutover_seq+1；late-outcome 判别 `outcome.seq ≤ cutover_seq ⇒ quarantine`；跨进程单调由 store 序号空间保证，禁止 run_id 哈希/进程内计数器——b93 B3）, `quiesced_dispatches: list[str]` | escape executor | executor/escape, executor, report |
...
>> **Prism:** 已复核 commit 982371b：§1e 两级占位符语义（基础封闭集一级展开 + TARGET 位置三个 version_scheme 键二级展开 + 范围外 fail-closed）机械可执行，两份物化合同 operations steps 与 schema 自洽，Devon 无需选择 schema。验证通过。
...
- `trac status` 新增稳定字段：`stage=M-VERIFY|M-SECURITY|awaiting_release|M-PUBLISH|M-MILESTONE|released|retry_tail|delayed|returned|cancelled`、`candidate=<full SHA>`、`full_reuse=full_f|full_rerun`、`ci=bound|mismatch|missing|stale|needs_attention`、`prism=pass|fail`、`security=passed|failed|unknown`、`preview_digest=…`、`decision=release|delay|return`、`publish=<op:done|reconciled_skip…>`、`terminal=released|cancelled`、`known_issues=[…]`、`repair=in_place round=<n>/3`、`human_return=<actor>→<to>`、`evidence.staled=<count>`、`barrier=established`、`late_outcome=quarantined`、`frozen_tests=unfrozen`、`needs_attention: <area> <reason> next=…`（E-03 形态）、阻断时 `blocked: <reason>`（§1.0.14 恢复分类，任何非通过出口都有可观察下一步）。**escape 专属五片段（`human_return`/`evidence.staled`/`barrier`/`late_outcome`/`frozen_tests`）的渲染函数归 escape 实现任务所有（b93 B1 函数级 carve）**：由单一装配点（如 `_escape_status_fragments(state)`）产出片段字符串，status 行总装与 release 侧片段归 release CLI 面任务；两侧函数级不相交。

### 2d. 通用逃生门（FR-0287；return 为既有子命令升级，abandon 为待实现 foundation task——散文引用形态）

- 通用回拨：`trac return --to <stage> --reason "<text>" [--confirm]`。**可回拨源阶段**（b93 裁定固化）：M-REQ-APPROVAL awaiting=approval；作者/评审阶段 awaiting=escalation（既有）；**M-IMPL 的 NEEDS_ATTENTION 与 DIAGNOSE/awaiting=escalation**（新增）；五个发布阶段 + AWAITING_RELEASE（注册后自动生效）。**目标**：任意上游 canonical 阶段——canonical 序唯一来源为 `tuple(machine._STAGES) − M-REQ-APPROVAL + release.RELEASE_STAGES`（M-REQ-APPROVAL 永不是目标；禁止第二份硬编码阶段表），「上游」= 序数严格更小，为兼容既有 escalation「current-or-earlier」允许等于当前阶段。Runtime 先建 escape barrier（cutover_seq 取事件库持久 seq）并 quarantine late outcome，再移动指针；回拨跨越已执行不可逆操作时终端先输出 `already_executed=[merge, tag=…, artifact=…]` 并**要求显式确认（`--confirm`）——无确认则非零退出、零事件、指针不动**。门禁/确认拒绝一律 `_err`（stderr）+ 零事件。成功后 status 显示 escape 五片段（§2b）：`human_return=<actor>→<to>`、`evidence.staled=<count>`、`barrier=established`、`late_outcome=quarantined`；**存在 human.return 且目标 canonical 序数 ≤ M-TEST 序数时**显示 `frozen_tests=unfrozen`（条件按序数判定，消除「回拨到 M-TEST 之前」的边界含糊，A-R2-1）。
...
- **合同**：SM-01.19——human.return 任意可回拨阶段（§2d 固化源集：M-REQ-APPROVAL awaiting=approval、作者/评审 awaiting=escalation、**M-IMPL NEEDS_ATTENTION 与 DIAGNOSE/escalation**、发布五阶段注册后自动纳入）→任意上游 canonical 阶段（canonical 序唯一来源 `tuple(machine._STAGES) − M-REQ-APPROVAL + release.RELEASE_STAGES`，禁止第二份硬编码表）。Runtime 先建 escape.barrier 并 quiesce 在飞 dispatch，late outcome 以 escape.late_outcome quarantine（禁止 checkpoint/publish/覆盖 State）；T 之后**实际存在**的证据桶 evidence.staled(reason=human_return) 不删除不复用（不存在的桶不发，防账本噪声）；回拨到 M-TEST 前解除测试冻结；跨越不可逆操作先报告清单并需 Human 显式确认（`--confirm`，无确认零事件不动指针），已执行操作经 reconcile 识别；Agent 咨询仅 advisory，Runtime 永不阻止 Human。
- **cutover_seq 唯一算法（b93 B3）**：writer_lock 内取该 run 事件库当前最大持久 seq 为 cutover_seq，`escape.barrier_established` 自身 seq = cutover_seq+1；late-outcome 判别 `outcome.seq ≤ cutover_seq ⇒ quarantine`；跨进程单调由 store 序号空间保证；run_id 哈希/进程内计数器实现禁止（实测两进程同值，违反 successive barriers strictly increase）。
- **事件顺序（writer_lock 内原子，顺序即合同顺序）**：门禁校验（拒绝→`_err`+零事件）→ 不可逆报告（非空无确认→`already_executed`+零事件不动指针）→ `escape.barrier_established` → `evidence.staled`（仅存在桶）→ `human.return`（双轨 payload，§1a）→ escape status 五片段渲染（§2b B1 carve）。真实 quiesce（barrier 后在飞 dispatch 的取消/冻结）与 escape.late_outcome 拦截已登记为 **deferred acceptance item**（AC-FR0287-02 的 quiesce 半句）：归 architecture §1.0.14.1 batch 2 后续任务（escape kernel/executor 落地，含 executor dispatch 循环消费 cutover），由 PLANNING 注册（test-plan §8.1 同款条目）；本合同 batch 1 只要求 payload 契约（int cutover + list），防「字段存在即算 quiesce」假绿。
- **指针迁移落地（b93 B4）**：回拨到 M-TEST/M-IMPL 的真实指针迁移依赖 rollback 目标闭合集扩展（executor，现仅四文档阶段）与 `_on_stage_rolled_back` 按目标 StageDef.initial_substate 路由（M-TEST=DISPATCH、M-IMPL=BASELINE，禁固定 DRAFT 死局）——该落地归独立后续任务（batch 2，依赖 escape 实现任务），本合同的 batch 1 交付面为「门禁 + 事件序列 + 确认门 + escape status 片段」，指针迁移完成断言登记到该后续任务锚点层。
- **modules**：executor/escape.py, cli/main.py（return 面 + escape status 片段渲染）, executor, kernel/release.py。
