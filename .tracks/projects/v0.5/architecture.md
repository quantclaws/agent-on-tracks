...
- 模板 + 校验（templating.py + executor/validate.py）：不变既有行为；v0.5 扩展 task-plan.md 校验（DAG/scope/AC 覆盖）与 dispatch 物化校验。
- checks/ 增长轴（trace.py + reach.py）：不变。M-IMPL ISLAND_GATE_2 消费 `trac check reach`（工具本体由 v0.4 交付）。
- 质量守卫栈八类 + pre-commit + CI required checks：不变。v0.5 增补测试代码分层政策（BS-11）。
...
- 新增 `executor/taskgraph.py`：task graph 解析 + DAG 无环 / scope 不重叠 / required AC 覆盖闭合校验（纯函数）。
...
- `executor/validate.py` 扩展 `trac validate --file task-plan.md` 校验 DAG/scope/AC 覆盖。
...
- 质量守卫配置增补测试代码分层：`.flake8` per-file-ignores（tests/ 豁免 CCR001）、`pyproject.toml` [tool.pylint] per-file disable（tests/ 豁免 R0915/R0914）。
...
| 5 | `executor/validate.py` | task-plan.md DAG/scope/AC 覆盖校验 | FR-0180 | IF-IMPL-003 |
...
- `PLANNING`：产出 `dispatch_agent(role=archer, substate=PLANNING)` command，携带 task-plan.md 模板 + 设计文档路径。Archer outcome 后由 executor 产出 `outcome.received` + `taskgraph.committed`。decide 路由：validate pass → ISLAND_GATE_1；validate fail → 重派 Archer（≤3 升级）。
...
- `GREEN_COMMIT`：产出 `create_green_commit` command。executor 创建正式 commit G（parent=B + trailers Tracks-Task/Tracks-Attempt/Tracks-R），产出 `green.committed`。decide 路由：→ REFACTOR。
  ...
  -> tracks.cli.main:cmd_run
    -> Executor(store, repo, run_id).run_loop()
      ...
      -> Executor._execute(cmd, state)
        ...
        -> _do_validate_taskgraph  # taskgraph.parse + taskgraph.validate_dag/scope/ac_coverage
        ...
        -> _do_create_green_commit # rgr.create_green_commit (parent=B + trailers)
...
- **FR-0180（task-plan/task-log）**：owner=executor/validate.py, surface=`trac run`+`trac validate --file task-plan.md`, composition=_do_validate_taskgraph+validate_document, wiring=cli→validate→taskgraph.parse+taskgraph.validate, test=integration test_taskplan_validate + test_tasklog_progress, evidence=`trac validate --file task-plan.md` 校验 DAG/scope/AC 覆盖 + task-log.md 写入, IF-=IF-IMPL-002, IF-IMPL-003, IF-IMPL-007。
...
- `tracks/executor/taskgraph.py` — task graph 解析 + DAG/scope/AC 覆盖校验桩：`parse_taskgraph` / `validate_dag` / `validate_scope` / `validate_ac_coverage` 签名，行为体 raise NotImplementedError("IF-IMPL-003")（kind: stub）
...
- `tracks/executor/worktree.py` — 三 worktree 方案桩：`create_devon_worktree` / `create_gate_worktree` / `create_test_authority_worktree` / `combine_for_gate` / `cleanup_worktree` 签名，行为体 raise NotImplementedError("IF-IMPL-006")（kind: stub）
...
### 3.1 task graph 解析（executor/taskgraph.py）

- **解析**：纯 Python 标准库（re/pathlib/dataclasses）。解析 task-plan.md 模板的 Task List 表格（ID / Task description / Related test / Target file / Depends on / Parallel marker / Status）+ Dependency Graph ASCII。
- **DAG 校验**：拓扑排序检测环（Kahn's algorithm）。scope 不重叠检查（manifest 白名单集合交集为空）。AC 覆盖闭合检查（每个 required AC 至少被一个 task 的 IF- 集合覆盖——读取 test-plan §8 AC Coverage 表 + task-plan 中每个 task 声明的 IF- 集合）。
- **取舍**：不引入 networkx 等图库（项目零运行时依赖）。Kahn's algorithm 用标准库 collections.deque 即可实现。
- **风险**：task-plan.md 表格格式可能不规则。缓解：解析器容错（跳过空行/注释行），格式错误产出明确校验失败消息。
...
- **G commit 创建**：`git commit-tree` 或 `git commit` with `--trailer` 选项（Git ≥ 2.32 支持 `--trailer`；项目 .gitignore 不影响 refs/trac/）。trailers：`Tracks-Task: {task_id}` / `Tracks-Attempt: {attempt}` / `Tracks-R: {r_sha}`。
...
- **分层机制**：quality_gate.py 按 changed_paths 分类文件为生产代码（tracks/ 下非 tests/）或测试代码（tests/ 下），分别执行不同检查集。配置文件（`.flake8` per-file-ignores、`pyproject.toml` [tool.pylint] disable）与 quality_gate.py 程序执行继承同一分层政策。
- **取舍**：不在 pre-commit 中分层（pre-commit 对所有文件执行全部检查，保持 v0.4 行为不变）；分层只在 GREEN_GATE/REFACTOR_GATE 中由 quality_gate.py 程序执行。代价：pre-commit 与 GREEN_GATE 的检查行为略有差异（pre-commit 对测试代码也跑 CCR001/R0915/R0914），但 GREEN_GATE 的分层是 spec 明确要求（BS-11）。
...
### 4.2 质量守卫栈（八类，继承 ARCH-004 canonical 配置 + v0.5 增补分层）
...
| 2 | 认知复杂度 | flake8==7.3.0 + flake8-cognitive-complexity==0.1.0 | `.flake8` | CCR001 ≤ 15（生产代码）；tests/ 豁免（per-file-ignores） | pre-commit + CI `lint` |
...
v0.5 增补的分层执行（BS-11）：
- `.flake8` 增补 `per-file-ignores = tests/*:CCR001`（测试代码豁免认知复杂度）。
- `pyproject.toml` `[tool.pylint]` 增补 `disable` per-file 配置（测试代码豁免 R0915/R0914）。
- GREEN_GATE/REFACTOR_GATE 中 `quality_gate.py` 按生产/测试代码分别执行检查集（程序执行，与配置文件继承同一分层政策）。
...
- **quality_gate 分层只在 GREEN_GATE/REFACTOR_GATE 执行**：pre-commit 仍对所有文件执行全部检查（v0.4 行为不变）。代价：pre-commit 与 GREEN_GATE 对测试代码的检查行为略有差异。理由：pre-commit 是本地提交前检查（全量），GREEN_GATE 是 task 级门禁（分层）。
