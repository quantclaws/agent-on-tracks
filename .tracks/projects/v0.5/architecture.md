...
- 失败（测试回归或质量门禁不通过）按 DIAGNOSE 路由（impl_defect 或 test_defect，与标准 RGR 一致）。其中 impl_defect 回 GREEN 的修复写入受本 task manifest 白名单（FR-0060）与越界审计（FR-0170）约束，按缺陷 owner 模块是否在本 task scope_boundary 内分两支：owner 模块在 scope 内的 task（T-001..T-005/T-008/T-009/T-011）由本 task 直接修复；owner 模块被其他 task 独占的跨模块验证 task（T-006/T-007/T-010/T-012）按 §5.1 记录的已接受降级路径处理——越界修复写入被原子回滚（over_reach），attempt 烧尽（≤3）后 awaiting_human（escalation），由 Human/Runtime 在 owning task 上下文修复后重派，该降级是显式设计行为而非隐性行为。
...
> **Prism [RESOLVED]:** PRISM-PLAN-R1-01 [severity=blocker] [artifact=.tracks/projects/v0.5/tasks.json T-002 / tasks.md line 23] [AC=AC-FR0020-01..04, AC-FR0030-01..03, AC-FR0180-01..05, AC-FR0220-01..04] T-002 的 scope_boundary 含幽灵路径：'tracks/executor/baseline.py' 在工作树中不存在；baseline digest 模块实际位于包根 tracks/baseline.py（本行六元组 owner=baseline.py 所指，且 FR-0020 各六元组 owner 为 executor/m_impl_runtime.py / kernel/m_impl.py / executor/worktree.py，均不在 tracks/executor/ 下的 baseline.py）。后果：tasks.json 是机器真相源与 TASK_DISPATCH per-task manifest 白名单来源（FR-0060），白名单将授权一个不存在的路径、并遗漏真实模块 tracks/baseline.py——T-002 期间对真实模块的任何修正性写入会被判越界，而误建 executor/baseline.py 重复模块反而被判合法，破坏任务图↔锁定架构的语义闭包。Runtime validate 只做结构校验（scope 不重叠，FR-0180-05：语义不由 Runtime 裁决），故未被捕获。预期修订（PLANNING/Archer，最小修复）：tasks.json T-002 scope_boundary 修正为 'tracks/executor/taskgraph.py, tracks/baseline.py'（如需完整覆盖 FR-0020 验证范围可另加 tracks/executor/m_impl_runtime.py，注意与 T-003 scope 不重叠），Runtime 重新投影 tasks.md 后回本线程闭环。
>> **Archer:** PRISM-PLAN-R1-01 addressed. tasks.json T-002 scope_boundary 已按预期修订：tracks/executor/taskgraph.py + tracks/executor/baseline.py（幽灵路径）→ tracks/executor/taskgraph.py + tracks/baseline.py（真实模块）。未追加 tracks/executor/m_impl_runtime.py：该路径已在 T-003 scope 内，追加违反 FR-0180-05 两两不交；FR-0020 验证失败落在 T-002 外 owner 模块的路径由 §5.1 跨模块降级条款覆盖（见 T-003 线程回复）。校验证据：parse/dag/scope/structure/issues/ac_coverage/island_closure 七项 validator 全部通过。tasks.md 投影由 Runtime commit_taskgraph 后重新生成。
...
- **跨模块 verification-only task 的修复边界（已接受降级路径）**：T-006/T-007/T-010/T-012 验证的合同六元组 owner 模块（executor/m_impl_runtime.py、executor/executor.py、kernel/m_impl.py、kernel/machine.py、executor/rgr.py、store/store.py、cli/main.py）全部已被其他 task 独占：batch-1 验证任务 T-001/T-003/T-004 分别持有 kernel/m_impl.py、m_impl_runtime.py、rgr.py，标准 RGR 任务 T-014/T-015/T-016 需要对 machine.py、store.py、executor.py、cli/main.py 的 GREEN 写入权；FR-0180-05 的 scope 两两不交约束使跨 task 共享不可能，而把基线验证并入 batch-2+ RGR 任务既破坏"基线验证先于新实现"的分层（§1.0.3），又混淆 verification-only 与标准 RGR 的 task 语义及 issue 对应关系。代价（已接受降级路径，与 §1.0.3 失败路由对齐）：这四个 task 验证失败且缺陷位于 owner 模块时，impl_defect 修复写入会被 manifest 越界审计（FR-0170）判 over_reach 并原子回滚，attempt 烧尽（≤3）后进入 awaiting_human（escalation），由 Human/Runtime 在 owning task 上下文修复后重派。理由：预实现合同（fa19f11）已通过既有 integration 测试，验证期失败是低概率回归信号，escalation 恰是正确的升级语义；为低概率修复路径重构 DAG（合并 task/issue 对应、延迟基线验证到新实现之后）成本与风险不成比例。test_defect 路径不受影响——四个 task 的新建 unit 测试文件均在自身 scope_boundary 内。
- **ground truth 不适用**：v0.5 是行为正确性（状态机转移、事件序列、git 操作），无算法正确性/规则正确性/计算结果正确性需要独立验证。test-plan §3 判定不适用，不创建 tests/ground_truth/。

> **Prism [RESOLVED]:** PRISM-PLAN-R1-02 [severity=blocker] [artifact=.tracks/projects/v0.5/tasks.json T-006/T-007/T-010/T-012 scope_boundary / tasks.md lines 75,88,127,153] [AC=AC-FR0140-01..03, AC-FR0150-01..04, AC-FR0160-01..04, AC-NFR0010-01..02, AC-FR0200-01..02, AC-NFR0020-01..02, AC-NFR0030-01..03] 本任务图中 4 个 verification-only task 的 scope_boundary 只含各自新建的 unit 测试文件，而它们验证的合同六元组 owner 模块全部不在本 task manifest 内：T-006 验证 FR-0140/0150（owner=executor/m_impl_runtime.py、executor/executor.py、kernel/m_impl.py）；T-007 验证 FR-0160/NFR-0010（owner=m_impl_runtime.py、kernel/m_impl.py、kernel/machine.py）；T-010 验证 FR-0200（owner=executor/executor.py、executor/rgr.py）；T-012 验证 NFR-0020/0030（owner=store/store.py、kernel/machine.py、executor/executor.py、cli/main.py）。后果：§1.0.3 明确规定 verification-only 失败按 DIAGNOSE 路由、impl_defect 与标准 RGR 一致回 GREEN 由 Devon 修复——但 GREEN 修复受 per-task manifest 白名单（FR-0060）与越界审计（FR-0170，over_reach 回滚 offending paths）约束，且 FR-0180-05/tasks.py validate_scope 要求全任务 scope 两两不交，本 task 无法合法写入待修复模块：任何真实 impl 修复会被原子回滚并白烧 attempt（≤3 次后 awaiting_human/escalation），§1.0.3 的失败语义在这 4 个 task 上结构性不可兑现。对照 T-001..T-005/T-008/T-009/T-011（scope 含被验证生产模块）可见分区不一致。预期修订（PLANNING/Archer，二选一或组合）：(a) 重新分区——把 T-006/007/010/012 的 AC 集合并入 owning task 或重排 scope，使每个 verification-only task 验证的合同可在自身 manifest 内修复；(b) 对跨模块合同（如 FR-0020 owner 横跨 m_impl_runtime/kernel/worktree）无法单 task 承载的部分，在 architecture.md §5.1/§5.2 如实记录'验证失败→越界修复→attempt 烧尽→escalation'为已接受降级路径（并与 §1.0.3 的 GREEN 路由表述对齐），不得留成隐性行为。修订后 Runtime 重新投影 tasks.md 并回本线程闭环。
>> **Archer:** PRISM-PLAN-R1-02 addressed：采用选项 (b) + 组合说明。结构事实：四个 task 的 owner 模块全部被其他 task 独占且不可让渡——kernel/m_impl.py∈T-001、m_impl_runtime.py∈T-003、rgr.py∈T-004（batch-1 验证任务）；machine.py/store.py∈T-014、cli/main.py∈T-015、executor.py∈T-016（标准 RGR 任务需要 GREEN 写入权）。FR-0180-05 两两不交使跨 task 共享非法；选项 (a) 只有两条出路——并入 batch-1 验证任务（owner 分属多 task，AC 集无法完整归属任何一个）或并入 batch-2+ RGR 任务（破坏 §1.0.3 基线验证先于新实现的分层，并混淆 verification-only 与标准 RGR 的 task 语义及 issue 对应）。故按预期修订选项 (b) 落地：(1) §1.0.3 第四条 bullet 重写——impl_defect 回 GREEN 的修复按 owner 模块是否在本 task scope_boundary 内分两支，跨模块验证 task（T-006/T-007/T-010/T-012）显式指向 §5.1 降级路径，不再是无条件 GREEN 路由表述；(2) §5.1 新增 bullet【跨模块 verification-only task 的修复边界（已接受降级路径）】：记录 owner 占用事实、两两不交约束、降级语义（over_reach 原子回滚 → attempt 烧尽 ≤3 → awaiting_human/escalation → Human/Runtime 在 owning task 上下文修复后重派）与取舍理由（fa19f11 预实现合同已通过既有 integration 测试，验证期失败为低概率回归信号，escalation 是正确升级语义，重构 DAG 成本/风险不成比例）；(3) §5.2 新增风险条目【verification-only 跨模块修复不可达】及缓解（test_defect 路径不受影响：四个 task 的新建 unit 测试文件均在自身 scope_boundary 内）。scope 不新增生产模块（两两不交下无合法增量），仅按 R1-03 去版本前缀。校验：七项 validator 全部通过。
...
- **verification-only 跨模块修复不可达**：T-006/T-007/T-010/T-012 验证合同的 owner 模块不在自身 manifest 内（scope 两两不交的必然结果，见 §5.1 已接受降级路径）。缓解：预实现合同已由既有 integration 测试覆盖（fa19f11），验证期失败为低概率回归信号；over_reach 原子回滚保证不产生越权写入；escalation 把修复显式路由到 Human/Runtime 的 owning task 上下文而非静默丢失；test_defect 路径（测试文件在自身 scope 内）不受该限制。
