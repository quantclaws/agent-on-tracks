...
## D-38. 脚手架产物归属收窄：ci.yml/lint 配置由 Archer 独立完成，不留 TODO(devon)

（用户裁定 2026-08-16，收窄 D-24 条款②/D-25）

Archer 作为开发项目 lead，职责含为团队搭建 scaffold——确定开发语言、技术栈、项目骨架、lint 配置、CI workflow。ci.yml、lint 配置这类产物本身输出文件不大，但与项目中其它部分关联很强，交给 Devon 做 RGR 效率很低。故裁定：

- **ci.yml、lint/pre-commit 配置、hook 脚本、声明性质量守卫配置**由 Archer **独立一次性完整交付**，Prism 审核，**不在骨架里留 `TODO(devon)` 标记给 Devon 补**。
- Archer 完成的项目骨架必须能够通过每一次 build（即骨架自身可运行、CI 可过）。
- D-24 条款②"声明性质量守卫配置由 Archer 按 Scaffold 宣言物理交付"维持，但**进一步收窄**：Archer 交付的是**完整可运行产物**，不是留待 Devon 补全的契约骨架。
- D-25 "CI workflow 由骨架补全"作为 foundation task 的条款**就此作废**——ci.yml 不再作为 Devon foundation task，Archer 一次写完。
- 实证：run 01KZTHE7RMZE6110PK9C54K1E2 T-017 manifest.allowed_paths=['.github/workflows/ci.yml']，Archer 在 ci.yml 骨架里留 6 处 `# TODO(devon)` 标记让 Devon 补 step 实现，导致 Devon RGR 轮次反复触发 audit/regression/evidence 问题，效率极低。

## D-41. 测试执行选择语义：R1/T-HIST 夜间回归 / R2/T-DELTA 本地差分执行 / FULL 链台账收敛

（用户裁定 2026-08-23；canonical 落点：flow.md §9.0、§9、§10.5、§11、§8.3-4；v3 修订同日，supersede v2；v5/v6 同日增补——v6=逐节点结果机器可读通道）

- **术语定义**：
  - **R1/T-HIST（历史回归节点）**：继承自既往版本的既有测试节点（unit/integration/e2e）。nightly CI 运行**当前完整 FULL 套件**（R1+R2，unit/integration/e2e——历史回归责任强调 R1/T-HIST，但 nightly 套件不是仅 R1 子集），结果在未来被取回（result fetch future），**不是当前本地门禁**；M-TEST 本地永不执行 R1 全量套件（M-IMPL 的 SELECT_TASK/FULL 链合法执行 R1 节点）。
  - **R2/T-DELTA（当前版本增量节点）**：新增节点、或同一 nodeid 节点源/体 digest 变化——分类**恰好且仅为**这两类（v3 简化：不再含 support/fixture 依赖声明归因）；分类按**逐节点**源/体 digest（非整文件 digest——同文件未变兄弟节点保持 R1/T-HIST）；**M-TEST** 本地执行只限 R2/T-DELTA。变更的 support/fixture 仍参与全量 collect/import（保证可 import），当前 R2 测试在其上执行；未变历史行为不被本地重验——其回归等待 M-IMPL FULL 链与 nightly CI（operator ruling）。
  - **REMOVED（fail-closed 测试资产删除）**：baseline 历史节点缺失于全量 collect——不静默注销，路由 test contract defect（→Shield）并阻断 M-TEST 退出；本版无授权删除路径。
  - **selection/evidence identity**：一次选择的身份 = 选择依据（task IF 集合/变更影响集/delta 声明）+ 被选节点集合 + baseline/commit；每条证据绑定节点身份 + selection identity + attempt + actor。上游变化使受影响的选择与证据 stale。
  - **失败台账（event ledger）状态**：`OPEN → CLASSIFIED → FIXED → PROVEN`，上游变化置 `STALE`；闭合转移集另含 `FIXED → OPEN`（证明失败：同签名重开条目重分类/重修）与 `PROVEN → OPEN`（后续全量复现已 PROVEN 同一签名）；未知/缺失/非法转移一律 fail-closed（不得视为干净）。台账 append-only 写事件（WAL），重启由回放重建。
- **M-TEST 时序（v3 重排）**：`WRITE → 全量 COLLECT → Runtime RED_CHECK(SELECT_R2) → PRISM_REVIEW → EXIT`。非法 RED 在 Prism 之前由 Runtime 路 DIAGNOSE/WRITE 处理完毕——Prism 评审时消费的是**当前树**的 `red.validated` 身份证据，再独立运行 counterexample kill；Prism 不把重跑普通套件当评审手段。collect 是全量的（继承 + 当前节点全部可 collect；baseline 历史节点缺失于 collect = fail-closed 测试资产删除）；M-TEST 执行只限 R2/T-DELTA 并逐节点判定合法 Red；**M-TEST 永不本地跑全量套件**；feature M-TEST 的 R2 选择集为空 = fail-closed（hotfix unit-only 显式增量声明旁路保留，FR-0244）。当前完整 FULL 套件夜间回归归 nightly CI，结果未来取回，不是当前本地 gate。
- **M-IMPL GREEN**：targeted unit（来自 task 不可变 R commit 的 Runtime 捕获 RED artifact manifest 与 GREEN-touched unit 文件——test-plan §8 IF 行只含 integration/e2e，不承载 unit 归属）+ integration（按 task IF 归属选择；不再要求"全部历史单测"）。REFACTOR：identity 未变复用 Green 证据；有变化按同 scope 重跑。
- **全部 task 完成（FULL 链）**：FULL = unit + integration + e2e；首轮 **FULL_1**；失败记入事件台账（身份=(node, failure_signature)）。修复循环是**逐条目**的（v3）：每条目 `OPEN→CLASSIFIED→FIXED` 后立即对其运行确定性 SELECT_DIFF——通过则 `FIXED→PROVEN`；同签名失败则 `FIXED→OPEN` 重分类/重修；某条目推导不出可靠差分时回退 FULL，该次 fallback 的结果逐条目落转移（通过条目 FIXED→PROVEN、同签名失败条目 FIXED→OPEN），fallback 干净可直接充当 FULL_F。全部条目 PROVEN 且无 STALE 后跑 **FULL_F**；FULL_F 出现失败则重复循环直至干净——**全链循环不设预算**（per-agent attempt 预算照旧）。多条目 union/batching 仅在每条目的选择/证据仍可单独归因时作为可选优化，不作为规范语义。
- **M-VERIFY**：candidate 相对干净 FULL_F 未漂移时**复用**并等待既有候选 CI 门禁的 `ci.run_observed(passed)` API 回读，**不重复本地全量**；漂移/stale 才重跑。（nightly 结果取回是另一回事，明确推迟到 future fetch issue。）
- **并发所有权与原子实现切片（v3）**：测试命令由 **Archer 的 machine contract 独家定义**——扁平 [unit]/[integration]/[e2e] 段的 run 与 run_selected 命令字符串，worker/dist 并发 flag 内嵌于命令字符串（无独立 workers/dist 键）；run_selected 缺失 = contract_error fail-closed（Runtime 不向 run 追加 nodeid、不合成调用），Runtime 永不注入并发。validator schema 激活、project.toml 合同扩展（[unit]、全部 run_selected、[nightly]）与 prompt/runtime 变更是**一个原子实现切片**（经正常 Tracks pipeline 交付）；本文档轮不拥有 `.tracks/projects/project.toml` 的并发工作树编辑（该编辑由另一授权 agent 持有）；缺键 fail-closed 自 schema 切片激活起生效。
- **v5 增补（同日，评审 findings 收敛；supersede 前文冲突处；canonical addendum: quantclaws/agent-on-tracks#47 issuecomment-5382394586）**：① **R1 快照机制可执行**——进入 M-TEST 时、本 run 首个 Shield WRITE 派发之前，Runtime 对继承测试树（prior-to-Shield 当前工作树）做 unit+integration+e2e 全量 collect 并持久化 `test.baseline_captured`（stamped baseline/tree identity + 逐节点 digest blob）；WRITE 后既有全量 COLLECT 以该快照为唯一分类 baseline——来源是 prior-to-Shield 当前 run 的 stamped capture，非既往事件猜测、非 WRITE 后可变树重算；首次项目可合法空集（缺失 capture ≠ 空 baseline，无 passed 快照 = fail-closed）；capture 失败 = 上游/设计/合同缺陷 fail-closed 路由（不静默空置 R1、不重派 Shield）；WAL/replay 复用同一 stamped capture。② **collect 三层**——全量 collect 明确含 `[unit]`（历史 unit 计入 inherited_r1 分类）；SELECT_R2 执行仍仅限实际 R2 节点（通常 Shield int/e2e；历史 unit 永不进入 M-TEST 执行记录）。③ **命令解析 API 修订**——`resolve_selected_command(template, nodes, cwd)->argv`、`audit_no_concurrency_injection(expected_argv, actual_argv, cwd)->bool`（两侧同一 argv0 解析后逐字比对）与组合形态 `audit(template, nodes, actual_argv, cwd)`；被选节点显式参与期望侧展开。④ feature 空 R2 fail-closed 与 REMOVED 阻断 M-TEST 的 RED 覆盖不再推迟——经 baseline capture 事件/blob seam 与宿主旅程构造（integration 层）。
- **v6 增补（同日，逐节点结果机器可读通道；supersede 前文冲突处）**：machine contract 的每条 `run` 与 `run_selected` 模板各含 `{result}` 占位符**恰好一次**（run_selected 另含 `{nodes}` 恰好一次），指 Runtime 提供的唯一可写 JUnit XML 路径；Archer 把结果写入 flag（pytest 即 `--junitxml={result}`）与并发 flag 一样内嵌于命令字符串——Runtime 替换封闭集 = `{nodes}`/`{result}` + cwd/argv0 解析，**永不注入 junit flag**。执行结果的权威是 JUnit XML：testcase 身份恰好覆盖被选集（全量 run 时=collect FULL 全集）；缺失/畸形/重复身份/被选缺席/多余节点 = contract_error fail-closed；stdout/stderr 仅为日志，永不作逐节点分类权威。逐节点合法 Red 从 testcase failure/error 详情推导（pass/skipped/xfailed 遵循既有合法 Red 规则：被选 R2 pass/skip = 非法意外通过）；FULL 链台账消费同一份归一化逐节点记录。结果生命周期：路径在 Runtime temp/blob staging、按 run/command 唯一、不入树身份；temp 清理前归一化持久化为 outcomes blob（事件携 `outcomes_ref`）；WAL replay 无已持久化结果即重跑该命令。API 相应修订：`resolve_selected_command(template, nodes, result_path, cwd)`、组合形态 `audit(template, nodes, result_path, actual_argv, cwd)`；contract loader 校验占位符计数（缺 `{result}`/`{nodes}` 或出现多次 = contract_error）。
...
| D-35 ◆ | 2026-08-15 | 评审意见按阶段分通道持久化：M-DESIGN 保持文档锚定（git 为 source of truth）；M-TEST/M-IMPL 代码评审单独成文入 blobs/，manifest 增加结构化字段 | 用户裁定（必须做、暂不阻塞、无 backlog 故记录于此）：初始 manifest 协议只考虑 M-DESIGN（评论可内嵌 markdown 正文，经 trac discuss 锚定+checkpoint diff 可审）；但 M-TEST/M-IMPL 评审对象是 Shield/Devon 代码，意见无法嵌入代码，必须单独成文。设计：① M-DESIGN 不变；② M-TEST/M-IMPL 的 Prism manifest 增加 review_summary（一句话，≤140 字符，manifest 只放摘要不放正文）、findings[]（id/severity/defect_classification/criterion/artifact/summary）、review_body（正文）——result_checkpoint 将 body 经 write_audit_blob 落 blobs/{sha256}，prism.verdict 事件行内保留 summary+findings+review_ref（引用地址），超 BLOB_THRESHOLD(8KB) 时 store 自动 $ref；③ machine._on_prism_verdict 把 review_ref 一并写入 last_failure 供重派 evidence。实证：run 01KZTHE7RMZE6110PK9C54K1E2 seq664 prism.verdict payload 无任何意见字段（协议未要求），Shield attempt 2 evidence 带旧 non_zero_exit（叠加 42s 版本竞态，trac run 05:22:22 启动早于 f10dca3 05:23:04）；result_checkpoint.py:255 透传与 machine 侧已就位，仅待 Prism.md manifest 约定补字段，DB 零改动；规范追踪见 .tracks/wiki/spec-d35-review-persistence.md（FR-0240 + AC-FR0240-01..06，v0.6 M-SPEC 并入）；触发条件：进入 M-IMPL 前复核代码评审的文档锚定补偿通道是否成立（M-TEST 有 test-plan.md 天然载体，M-IMPL 锚设计文档语义错位），不成立则本条升级为阻塞项（events 1510 行/672KB，blobs 137MB，容量充裕） |
| D-36 | 2026-08-15 | OOB（Out of Band）提交机制：runtime 视野外的人类/外围系统变更经显式 trailer 声明后接受——不审查、不计入 Agent 产出、不触发 over_reach；浅版先行（静默期 re-baseline + Tracks-OOB trailer + oob.accepted 事件），深版预留（dispatch 窗口内 git 历史归因，遍历 dispatch_ref..HEAD 按 trailer 排除 OOB commit） | 动机：run 01KZTHE7RMZE6110PK9C54K1E2 实证 dispatch 窗口内外部 git 操作被迫排队/撤回（FR-11 修复延后提交、SC-D35 部署中断、decisions.md 差点触发 over_reach 回滚 Shield），否定式归因（baseline 快照后一切变化归 Agent）需机制化第三通道；信任模型（用户裁定 2026-08-15）：信任 Agent 且 Agent 无 git 权限，逃逸/伪造 trailer 不作为设计约束；重放一致性依托 git trailer（append-only 天然可重放），OOB 接受需发 oob.accepted 事件保持事件流完整；build-gate（用户裁定 2026-08-17）：深版实施前必须以宿主项目场景（运行期并发人类提交触及 tests/、暂停数日后恢复时树漂移）重新论证价值，狗粮运维事故（如 dcec932 基线恢复）不构成独立立项依据——不发布用户用不到的功能；浅版之『tests/ 变更由下一次 M-IMPL baseline 冻结吸收』为既有周期机制的零代码语义说明，已写入 v0.5 FR-0170（commit 71ac6f4），非新功能 |
> ◆ = backlog（必须做、暂不阻塞、待排期实施）：D-34（stderr 摘要入 last_failure.evidence）、D-35（评审意见分阶段持久化，规范追踪 SC-D35/FR-0240）、D-36（OOB 提交机制）、D-39（infra 重派会话续传+前任告知，v0.6）。各项互相独立，均在 dispatch 窗口外静默期实施，逐项完成后消除对应 ◆。
| D-37 | 2026-08-15 | over_reach 处置流程缺口（8 步轮次方案）：run 01KZTHE7RMZE6110PK9C54K1E2 实证——Prism DENY 裁定正确（pyproject 属真越权、合规解法存在：hook 移入 tests/integration/），但 (a) 裁定结果不回流 Shield（adjudicate 只返回 bool，DENY 后仅回滚+计失败，Shield 每轮盲猜）；(b) 越权即整轮 force 回滚，合规测试文件一并丢失；(c) attempt 不重置，同因连发三轮烧 116 分钟 agent 时间后 escalation。完整 8 步方案（含 Shield 自接受免申诉分支）见 .tracks/wiki/spec-oob-adjudication-loop.md（SC 提案，v0.6 M-SPEC 并入；FR 编号立项时分配） | 用户裁定 2026-08-15：7 步轮次 + Shield 自接受分支；处置原则=回滚保留合规、拒绝理由必回流、申诉走 Prism、同意走 Archer 改白名单、裁定后新 loop 重置 attempt；本次 v0.5 人工干预推进 |
| D-38 | 2026-08-16 | 脚手架产物归属收窄：ci.yml/lint 配置由 Archer 独立完成，不留 TODO(devon) | 用户裁定：Archer 是开发 lead，scaffold（语言/技术栈/项目骨架/lint/CI）由 Archer 独立一次性完整交付 + Prism 审核；ci.yml/lint 输出小但关联强，交 Devon RGR 效率低；Archer 骨架必须通过每一次 build；D-25「CI workflow 由骨架补全作 foundation task」作废；实证 run 01KZTHE7RMZE6110PK9C54K1E2 T-017 ci.yml 留 6 处 TODO(devon) 致 Devon RGR 反复 audit/regression/evidence 问题 |
| D-39 ◆ | 2026-08-16 | infra 超时重派的会话续传与「前任告知」：① 轻版（确定性提示注入）——infra 类失败（timeout 流挂死/signal/provider_unavailable）重派同一 assignment 时，objective 追加 runtime 确定性已知的事实：「本派发前一实例运行 N 秒后被 runtime 超时终止，磁盘可能含其部分工作，baseline/pre_dirty_snapshot 哈希为权威，先验证再决定是否重做」；② 深版（会话续传）——OpencodeBackend 按 (role, substate, task) 跟踪 opencode session id，infra 重派时以 `--session <id>` 续传（记忆完整、断点续跑、最经济），session 不存在则回退全新派发。边界：仅适用 infra 重试；agent-attempt 重试（impl_defect 等）保持带诊断全新开始（已验证有效）；kernel 保持纯净，session 跟踪属 effects 层；续传提示语仍注入 runtime 事实，防「当时的计划」与已推进 baseline 冲突；与 D-33 相容（D-33 禁 elapsed 超时，本条依赖的是 D-23 不活跃看门狗，18f43bf 已落地） | 用户裁定 2026-08-16（backlog，v0.6，本版快速收尾）：实证 run 01KZTHE7 T-017——挂死会话被杀后 Devon 重派面对「前任」已完成的工作，需靠 Prism DIAGNOSE 379s 转述「工作已完成只缺 evidence JSON」，而该事实 runtime 确定性已知（kill 事件 + 工作区 diff）；e01390f/a302a2d/606dd4c/fbd2681/18f43bf 已完成 infra/agent 失败分层，本条为其自然延伸 |
| D-40 | 2026-08-17 | 运营者窗口纪律 + note-r1 处置（resolved-by-discipline）+ provider 归因纪律 | 用户裁定：① dispatch 窗口内运营者对工作区**零写入**（含 .tracks/ 文档与 git 操作）；窗口外写入由 pre-dispatch 快照吸收（FR-0170 豁免①既有行为）；违反窗口纪律的写入触发 fail-closed 属正确行为，按诊断路由处置。② spec-acc-amendment-note-r1 三个问题零代码绕过：1261 由 rollback 后 M-IMPL 重入的每任务 `r_tree_identity` 自动重冻结消解（瞬态、无持久基线可改）；1264 由窗口纪律消除触发器；1267（spec_gap）随二者条件性不存在；修订文本（71ac6f4）保留留档作 v0.6 D-36 输入，豁免③代码补丁（audit.py 路径排除）不部署，与 D-36 深版一并待宿主场景论证。③ provider 归因纪律：多 deployment 轮询下个别 429/rate limit 是正常背景噪声，诊断时不得作为根因结论——429 后 30 分钟挂死本身是本地（opencode/tracks）容错缺陷信号，归入 D-34/D-39 证据富化与重试治理范畴 |
| D-41 | 2026-08-23 | 测试执行选择语义：R1/T-HIST 夜间回归 / R2/T-DELTA 本地差分执行 / FULL 链台账收敛 | 用户裁定（v3 修订同日，supersede v2；v5/v6 同日增补）：M-TEST 时序 WRITE→全量 COLLECT→Runtime RED_CHECK(SELECT_R2)→PRISM_REVIEW→EXIT（非法 RED 在 Prism 前路 DIAGNOSE/WRITE，Prism 消费当前树 red.validated 证据 + 只做 counterexample kill）；全量 collect（baseline 节点缺失 = fail-closed 删除）、只本地执行 R2/T-DELTA（逐节点 digest 分类：新增∨节点 digest 变化，v3 移除依赖声明归因；变更 support 仍参与 collect/import，未变历史行为等 M-IMPL FULL/nightly）并逐节点合法 Red、永不本地全量套件、feature M-TEST 空 R2 fail-closed（hotfix unit-only 显式增量旁路保留）。FULL 链=FULL_1→事件台账（身份=(node,failure_signature)；闭合转移含 FIXED→OPEN 证明失败与 PROVEN→OPEN 复现 reopen）且修复循环逐条目：每条 FIXED 后立即确定性 SELECT_DIFF（过=PROVEN；同签名败=FIXED→OPEN 重修；不可证明回退 fallback_full 并逐条目落转移，干净可充 FULL_F）→全部 PROVEN 后 FULL_F 循环至干净（无全链预算、per-agent attempt 照旧；union/batching 仅可单独归因时为可选优化）。M-VERIFY 未漂移复用干净 FULL_F 等候选 CI ci.run_observed 回读（nightly 结果取回另案推迟），不重复本地全量。nightly 本版审计=contract 文件+validate 校验（dispatch/readback 事件待 future fetch issue）。validator schema 激活+project.toml 扩展([unit]/run_selected/[nightly])+prompt/runtime 变更=一个原子实现切片；本文档轮不拥有 project.toml 并发工作树编辑；缺键 fail-closed 自 schema 激活生效。测试命令由 Archer machine contract 独家定义（并发 flag 与 `--junitxml={result}` 内嵌命令字符串——每条 run/run_selected 各含 `{result}` 恰好一次、run_selected 另含 `{nodes}` 恰好一次；结果权威=JUnit XML 逐 testcase 身份恰好覆盖被选集，缺失/畸形/重复/覆盖不精确=contract_error fail-closed，stdout/stderr 仅日志；FULL 台账消费同一归一化 outcomes blob，replay 无持久化结果即重跑；run_selected 缺失即 contract_error 不合成调用），Runtime 永不注入并发与 junit flag |
| D-42 | 2026-09-19 | Agent 会话生命周期三层机制（修订 D-39 深版落地形态，修正 #44 简化版）：键=(run_id, role, work_unit, task_id)——work_unit 由 (role, substate) 派生（devon RGR 同任务同键、shield 写作/修复分单元、archer 设计/规划分单元、评审按 substate）；同键一律续传（infra/format/revise 统一，FR-11 证据注入纠偏——**取代 D-39 的 attempt-fresh 边界**）；派发前 ctx_tokens ≥ 85%×模型窗口触发压缩（`opencode run --session --command compact`，窗口从 `opencode models --verbose` 的 limit.context 解析），压缩失败或压缩后仍超阈值→弃会话新建；派发后从最后一个 step_finish.tokens.input 记录实际尺寸；miss/overflow/non_zero_exit 弃会话（裁定：runtime 负责传递 agent 所需信息——任务、verdict 等，agent 只重建自己的探索/思考/结论）；单回合中途超限→派发失败→infra 重派→派发前收缩→自愈闭环。完整设计：.tracks/projects/v0.9/oob/2026-09-19-session-lifecycle.md | 用户裁定 2026-09-19（三点：85% 阈值+读取 provider/model 上下文设置；non_zero_exit 保持清会话；同意同键续传取代 attempt-fresh）。动机：#44（per-agent 无界续传）在长 run 实证爆配额——run 01M2QTJB 2026-09-18/19：Archer 会话 1301 parts/累计 24M input/单回合 749K context/从未压缩 → 429 token plan exhausted → 6 次 no_envelope_block + exit-1 级联。带外实施（Tracks-OOB），决策补录本条 |
