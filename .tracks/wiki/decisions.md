...
> ◆ = backlog（必须做、暂不阻塞、待排期实施）：D-34（stderr 摘要入 last_failure.evidence）、D-35（评审意见分阶段持久化，规范追踪 SC-D35/FR-0240）、D-36（OOB 提交机制）。三项互相独立，均在 dispatch 窗口外静默期实施，逐项完成后消除对应 ◆。
| D-37 | 2026-08-15 | 测试辅助 hook 白名单 + Shield 构建配置只读 + manifest 失败路径回滚缺口登记：① [layout.shield] 增补 tests/_doc_gap_hook/ 与 tests/doc_gap_injection.py（PRISM-V05-R2-02 的 dispatch-window 注入 hook 是合法测试资产，白名单漏登致 run 01KZTHE7RMZE6110PK9C54K1E2 三轮 over_reach）；② Shield.md 增补构建配置只读条款（pyproject/.githooks/.flake8/tox/CI 不得写入，lint 门禁受阻走 self_report）；③ manifest_malformed/provider_unavailable 早退失败路径不做审计回滚——登记为已知缺口（残留由下轮 baseline 吸收、不构成 over_reach 误判，但会以 untracked 干扰 NO_DIFF 判定），修复随 D-36 OOB 一并排期 | 用户裁定 2026-08-15（早班 handoff 会话）：白名单漏登与 agent 自救越权是 over_reach 连发双根因；夜班 handoff 的残留污染机制描述经代码核实有误已勘误（baseline 每 dispatch 重新快照） |
