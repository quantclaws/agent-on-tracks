...
def _m_test_assignment_v04():
    return {"kind": "WRITE", "skills": ["tracks-discuz"],
...
def test_shield_write_manifest_extracted_from_final_text_event(
    ...
    monkeypatch.setenv("FAKE_OPENCODE_FINAL_TEXT",
                       f"\n  {json.dumps(payload)}  \n")
...
def test_shield_write_missing_required_field_fails(
    ...
    _set_shield_env(monkeypatch, "shield_test", docs, test_file)
    monkeypatch.setenv("FAKE_OPENCODE_FINAL_TEXT", json.dumps({
        "artifact_manifest": {"include": [{
...
def test_shield_write_non_json_final_text_fails(
        fake_opencode, host_repo, monkeypatch):
    from tracks import paths
    vdir = paths.version_dir(paths.tracks_home(host_repo), "v0.4")
    vdir.mkdir(parents=True, exist_ok=True)
