...
from tests._support.opencode_backend_fixtures import ARCH_WITH_SCAFFOLD as ARCH_WITH_SCAFFOLD
from tests._support.opencode_backend_fixtures import DESIGN_DOCS as DESIGN_DOCS
from tests._support.opencode_backend_fixtures import STANDIN as STANDIN
from tests._support.opencode_backend_fixtures import backend as backend
from tests._support.opencode_backend_fixtures import commit_doc as commit_doc
from tests._support.opencode_backend_fixtures import commit_trio as commit_trio
from tests._support.opencode_backend_fixtures import design_assignment as design_assignment
from tests._support.opencode_backend_fixtures import design_vdir as design_vdir
from tests._support.opencode_backend_fixtures import fake_opencode as fake_opencode
from tests._support.opencode_backend_fixtures import seed_thread as seed_thread
from tests._support.opencode_backend_fixtures import skill_path as skill_path
from tests._support.opencode_backend_fixtures import target_doc as target_doc
from tests._support.opencode_backend_fixtures import template_paths as template_paths
...
def test_operator_cancel_kills_process_group(fake_opencode, target_doc, host_repo, monkeypatch):
...
def test_jsonl_with_log_prefix_not_truncated(fake_opencode, target_doc, host_repo, monkeypatch):
...
def test_triage_does_not_require_target_diff(fake_opencode, target_doc, host_repo, monkeypatch):
    monkeypatch.setenv("FAKE_OPENCODE_BEHAVIOR", "no_edit")
...
def test_reviewer_open_discussion_requests_revision(
    ...
    target_doc.write_text(writer.start(text, 5, "Sage", "Which output policy?"), encoding="utf-8")
    subprocess.run(["git", "add", "story.md"], cwd=host_repo, check=True)
    subprocess.run(
        ["git", "commit", "-m", "add open discussion"],
        cwd=host_repo,
        check=True,
        capture_output=True,
    )
...
def test_in_repo_write_is_allowed_not_over_reach(fake_opencode, target_doc, host_repo, monkeypatch):
...
def test_archer_design_draft_accepts_the_doc_trio(
    ...
    out = backend(host_repo).act(
        "archer", "DRAFT", None, None, assignment=design_assignment("DRAFT")
    )
    assert out["status"] == "done"
    ...
    assert not (host_repo / ".opencode" / "skills" / "tracks-discuz" / "SKILL.md").exists()
...
def test_archer_respond_partial_docset_passes(fake_opencode, design_vdir, host_repo, monkeypatch):
    ...
    monkeypatch.setenv("FAKE_OPENCODE_DOCS", ",".join(map(str, docs)))
    out = backend(host_repo).act(
        "archer", "RESPOND", None, None, assignment=design_assignment("RESPOND")
    )
    assert out["status"] == "done"
    assert out.get("failure_class") is None
...
def test_archer_respond_no_change_still_fails(fake_opencode, design_vdir, host_repo, monkeypatch):
...
def test_archer_author_undeclared_scaffold_write_fails(
    ...
    monkeypatch.setenv("FAKE_OPENCODE_EXTRA", str(scratch))
    out = backend(host_repo).act(
        "archer", substate, None, None, assignment=design_assignment(substate)
    )
    assert out["status"] == "failed"
...
def test_prism_review_write_audit_stays_repo_root_trusted(
    ...
    monkeypatch.setenv("FAKE_OPENCODE_EXTRA", str(scratch))
    out = backend(host_repo).act(
        "prism", "PRISM_REVIEW", None, None, assignment=design_assignment("PRISM_REVIEW")
    )
    assert out["status"] == "done"
    assert out.get("failure_class") is None
    assert scratch.exists()
...
def test_design_agents_materialize_restore_human_agent(
    ...
    out = backend(host_repo).act(role, substate, None, None, assignment=design_assignment(substate))
    assert out["status"] == "done"
    assert human.read_text(encoding="utf-8") == "HUMAN AGENT\n"
...
def test_single_doc_stage_materializes_its_template_kind(fake_opencode, host_repo, monkeypatch):
    ...
    subprocess.run(
        ["git", "commit", "-m", "spec skeleton"], cwd=host_repo, check=True, capture_output=True
    )
    ...
    out = backend(host_repo).act("sage", "DRAFT", "spec.md", spec, assignment=assignment)
...
def test_missing_canonical_template_fails_loudly(fake_opencode, target_doc, host_repo, monkeypatch):
    ...
    assignment = {
        "kind": "DRAFT",
        "templates": ["spec", "no-such-template"],
        "skill": "tracks-discuz",
        "skill_version": "0.2",
    }
    out = backend(host_repo).act("scribe", "DRAFT", "story.md", target_doc, assignment=assignment)
    assert out["status"] == "failed"
    assert out["failure_class"] == "opencode_missing"
...
def test_materialize_templates_missing_kind_raises(host_repo):
    ...
    with pytest.raises(OpencodeError) as excinfo:
        backend(host_repo)._materialize_templates({"templates": ["bogus-kind"]}, infos)
...
def test_draft_own_thread_left_open_fails(fake_opencode, target_doc, host_repo, monkeypatch):
...
def test_draft_own_thread_resolved_passes(fake_opencode, target_doc, host_repo, monkeypatch):
...
def test_reviewer_revise_without_findings_fails(fake_opencode, design_vdir, host_repo, monkeypatch):
...
def test_multi_skill_materialization_and_cleanup_restores(host_repo):
    ...
    try:
        ...
        for info in infos:
            canonical = be._canonical.parent / "skills" / info["dest"].parent.name / "SKILL.md"
            assert info["dest"].read_bytes() == canonical.read_bytes()
    finally:
        for info in infos:
            be._cleanup(info)
...
def test_multi_skill_materialization_end_to_end(fake_opencode, design_vdir, host_repo, monkeypatch):
    ...
    docs = [design_vdir / name for name in DESIGN_DOCS]
    skills = [skill_path(host_repo, name) for name in ("tracks-discuz", "tracks-quality-guards")]
    monkeypatch.setenv("FAKE_OPENCODE_BEHAVIOR", "edit_docs")
    monkeypatch.setenv("FAKE_OPENCODE_DOCS", ",".join(map(str, docs)))
...
def test_single_skill_backward_compat(fake_opencode, design_vdir, host_repo, monkeypatch):
    ...
    monkeypatch.setenv("FAKE_OPENCODE_BEHAVIOR", "no_edit")
    monkeypatch.setenv("FAKE_OPENCODE_DOCS", ",".join(map(str, docs)))
    monkeypatch.setenv("FAKE_OPENCODE_SKILLS", str(skill_path(host_repo, "tracks-discuz")))
    out = backend(host_repo).act("prism", "PRISM_REVIEW", None, None, assignment=assignment)
    assert out["status"] == "done"
    assert not skill_path(host_repo, "tracks-discuz").exists()
    assert not (host_repo / ".opencode" / "skills" / "tracks-prism-design").exists()
...
def test_scaffold_declared_paths_parser():
    ...
    assert _scaffold_declared_paths(text) == {"pyproject.toml", "src/pkg/__init__.py"}
    # a bullet-shaped line in the NEXT level-2 section is not a declaration
    extended = text + "\n## 6. 附录\n\n- later.md — outside the manifest\n"
    assert _scaffold_declared_paths(extended) == {"pyproject.toml", "src/pkg/__init__.py"}
    ...
    assert _scaffold_declared_paths(unnumbered) == {"pyproject.toml", "src/pkg/__init__.py"}
...
def test_scaffold_declared_paths_strips_backticks():
    ...
    assert _scaffold_declared_paths(text) == {"code_stats/__init__.py", "pyproject.toml"}
...
def test_scaffold_declared_paths_strips_quotes():
    ...
    assert _scaffold_declared_paths(text) == {"code_stats/cli.py", "code_stats/counter.py"}
...
def test_scaffold_declared_paths_mixed_manifest():
    ...
    assert _scaffold_declared_paths(text) == {"code_stats/__init__.py", "pyproject.toml", ".flake8"}
