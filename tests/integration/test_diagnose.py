...
def test_test_defect_redispatch(trac, event_log):
    ...
    failed = [
        e
        for e in evs
        if e["type"] == "verdict.failed" and e["payload"].get("check") == "test_defect"
    ]
...
def test_stub_gap_no_human(trac, event_log):
    ...
    evs = event_log(run_id)
    failed = [
        e for e in evs if e["type"] == "verdict.failed" and e["payload"].get("check") == "stub_gap"
    ]
    assert failed
    ...
    human_between = [
        e for e in evs if e["type"].startswith("human.") and fail_seq < e["seq"] < roll_seq
    ]
...
def test_ac_gap_needs_human(trac, event_log):
    ...
    evs = event_log(run_id)
    failed = [
        e for e in evs if e["type"] == "verdict.failed" and e["payload"].get("check") == "ac_gap"
    ]
    assert failed
...
def test_spec_gap_needs_human(trac, event_log):
    ...
    evs = event_log(run_id)
    failed = [
        e for e in evs if e["type"] == "verdict.failed" and e["payload"].get("check") == "spec_gap"
    ]
    assert failed
...
def test_verdict_failed_payload(trac, event_log):
    ...
    failed = [
        ...
        and e["payload"].get("check") in ("test_defect", "stub_gap", "ac_gap", "spec_gap")
    ]
    assert failed
    p = failed[0]["payload"]
    assert "check" in p
