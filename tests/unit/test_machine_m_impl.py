...
DEVON_REFACTOR_DISPATCH = ("command.issued", {"command": {"kind": "dispatch_agent",
                                                           "params": {"role": "devon",
                                                                      "substate": "REFACTOR"},
                                                           "command_id": "C14"}})
DEVON_REFACTOR_DONE = ("outcome.received", {"role": "devon", "status": "done"})

REFACTOR_GATE_CMD = ("command.issued", {"command": {"kind": "run_refactor_gate",
                                                     "params": {"stage": "M-IMPL"},
                                                     "command_id": "C15"}})
REFACTOR_COMMITTED = ("refactor.committed", {"commit_sha": "ghi789"})

TASK_REVIEW_CMD = ("command.issued", {"command": {"kind": "run_task_gates",
                                                   "params": {"stage": "M-IMPL",
                                                              "gate": "TASK_REVIEW"},
                                                   "command_id": "C16"}})
TASK_REVIEW_PASS = ("verdict.passed", {"check": "task_review"})

PRISM_FINAL_DISPATCH = ("command.issued", {"command": {"kind": "dispatch_agent",
                                                        "params": {"role": "prism",
                                                                   "substate": "PRISM_FINAL"},
                                                        "command_id": "C17"}})
PRISM_FINAL_DONE = ("outcome.received", {"role": "prism", "status": "done"})
PRISM_FINAL_PASS = ("prism.verdict", {"verdict": "pass",
                                       "criteria_pack": dict(_M_IMPL_CRITERIA_PACK)})

COMPLETE_TASK_CMD = ("command.issued", {"command": {"kind": "complete_task",
                                                     "params": {"stage": "M-IMPL",
                                                                "task_id": "T1"},
                                                     "command_id": "C18"}})
TASK_COMPLETED = ("task.completed", {})

ISLAND2_CMD = ("command.issued", {"command": {"kind": "check_island_2",
                                               "params": {"stage": "M-IMPL"},
                                               "command_id": "C19"}})
ISLAND2_PASS = ("verdict.passed", {"check": "island_2"})

EXIT_CMD = ("command.issued", {"command": {"kind": "write_frontmatter",
                                            "params": {"stage": "M-IMPL"},
                                            "command_id": "C20"}})
...
def _full_single_task_cycle():
    ...
    return [
        ...
        DEVON_REFACTOR_DISPATCH, DEVON_REFACTOR_DONE,
        REFACTOR_GATE_CMD, REFACTOR_COMMITTED,
        TASK_REVIEW_CMD, TASK_REVIEW_PASS,
        PRISM_FINAL_DISPATCH, PRISM_FINAL_DONE, PRISM_FINAL_PASS,
        COMPLETE_TASK_CMD, TASK_COMPLETED,
        ISLAND2_CMD, ISLAND2_PASS,
        EXIT_CMD, STAGE_EXITED,
    ]
...
def test_prism_plan_revise_to_planning():
    ...
    s = state_of(BASELINE_CMD, BASELINE_FROZEN,
                 ARCHER_DISPATCH, ARCHER_DONE, TASKGRAPH_CMD, TASKGRAPH_COMMITTED,
                 ISLAND1_CMD, ISLAND1_PASS,
                 PRISM_PLAN_DISPATCH, PRISM_PLAN_DONE, revise)
    assert s.substate == "PLANNING"
    assert s.current_attempt == 1
...
def test_red_gate_fail_to_red():
    ...
    fail = ("verdict.failed", {"check": "red_invalid", "reason": "no fail",
                               "attempt": 1})
    s = state_of(BASELINE_CMD, BASELINE_FROZEN,
                 ARCHER_DISPATCH, ARCHER_DONE, TASKGRAPH_CMD, TASKGRAPH_COMMITTED,
                 ISLAND1_CMD, ISLAND1_PASS,
...
def test_prism_red_revise_to_red():
    ...
    s = state_of(BASELINE_CMD, BASELINE_FROZEN,
                 ...
                 RED_GATE_CMD, RED_VALID_PASS,
                 RED_CHECKPOINT_CMD, RED_CHECKPOINTED,
                 PRISM_RED_DISPATCH, PRISM_RED_DONE, revise)
    assert s.substate == "RED"
    assert s.current_attempt == 1
...
def test_refactor_no_change_to_task_review():
    ...
    s = state_of(BASELINE_CMD, BASELINE_FROZEN,
                 ...
                 GREEN_COMMIT_CMD, GREEN_COMMITTED,
                 DEVON_REFACTOR_DISPATCH, DEVON_REFACTOR_DONE,
                 REFACTOR_GATE_CMD, ("refactor.no_change", {"reason": "clean"}))
    assert s.substate == "TASK_REVIEW"
    assert s.refactor_done is True
...
def test_prism_final_revise_impl_to_green():
    ...
    s = state_of(BASELINE_CMD, BASELINE_FROZEN,
                 ...
                 REFACTOR_GATE_CMD, REFACTOR_COMMITTED,
                 TASK_REVIEW_CMD, TASK_REVIEW_PASS,
                 PRISM_FINAL_DISPATCH, PRISM_FINAL_DONE, revise)
    assert s.substate == "GREEN"
    assert s.current_attempt == 1
...
def test_multi_task_iterates_to_dispatch():
    ...
    events = [
        BASELINE_CMD, BASELINE_FROZEN,
        ARCHER_DISPATCH, ARCHER_DONE, TASKGRAPH_CMD,
...
def test_task_started_clears_diagnose_classification():
    ...
    events = [
        ...
        DEVON_GREEN_DISPATCH, DEVON_GREEN_DONE,
        GREEN_GATE_CMD,
        ("verdict.failed", {"check": "unknown_attribution", "reason": "unclear",
                            "attempt": 1}),
        ...
        PRISM_FINAL_DISPATCH, PRISM_FINAL_DONE, PRISM_FINAL_PASS,
        COMPLETE_TASK_CMD, TASK_COMPLETED,
        SELECT_TASK_CMD, ("task.started", {"task_id": "T2"}),
    ]
    s = state_of(*events)
    assert s.substate == "RED"
...
def test_refactor_gate_public_interface_to_diagnose_stub_gap():
    ...
    fail = ("verdict.failed", {"check": "public_interface", "reason": "broke API",
                               "attempt": 1})
...
def test_island_gate_2_full_suite_fail_to_diagnose():
    ...
    fail = ("verdict.failed", {"check": "full_suite", "reason": "e2e fail",
                               "attempt": 1})
    s = state_of(*base, fail)
    assert s.substate == "DIAGNOSE"
...
def test_red_failure_escalation():
    ...
    fail = ("outcome.received", {"role": "devon", "status": "failed",
...
def test_failed_devon_red_routes_to_diagnose_with_evidence():
    s = state_of(
        ...
        PRISM_PLAN_DISPATCH, PRISM_PLAN_DONE, PRISM_PLAN_PASS,
        SELECT_TASK_CMD, TASK_STARTED,
        DEVON_RED_DISPATCH, _devon_failed_outcome(),
    )
    assert s.substate == "DIAGNOSE"
    assert s.current_attempt == 1
...
def test_failed_devon_green_routes_to_diagnose_with_r_lineage():
    s = state_of(
        ...
        RED_CHECKPOINT_CMD, RED_CHECKPOINTED,
        PRISM_RED_DISPATCH, PRISM_RED_DONE, PRISM_RED_PASS,
        DEVON_GREEN_DISPATCH, _devon_failed_outcome(),
    )
    assert s.r_tree_identity == "abc123"
    assert s.substate == "DIAGNOSE"
    assert s.current_attempt == 1
...
def test_failed_devon_refactor_stays_in_refactor_retry():
    s = state_of(
        ...
        GREEN_GATE_CMD, GREEN_PASS,
        GREEN_COMMIT_CMD, GREEN_COMMITTED,
        DEVON_REFACTOR_DISPATCH, _devon_failed_outcome(),
    )
    assert s.substate == "REFACTOR"
    assert s.current_attempt == 1
    cmd = decide(s)
