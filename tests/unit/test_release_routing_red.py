...
def test_decide_release_stage_routes_six_commands():
    ...
    # Unfinished M-VERIFY stays decide-silent: the bounded chain re-drive
    # lives in the executor's loop entry (once per trac run) -- a decide
    # re-issue would spin against an unresolved attention.
    cmd = _route("M-VERIFY", subs["M-VERIFY"], _fresh_state(stage="M-VERIFY"))
    if cmd is not None:
        _fail(f"uncompleted M-VERIFY must route None (bounded executor re-drive), got {cmd!r}")
