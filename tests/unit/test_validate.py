...
def _story(tmp_path: Path) -> Path:
    p = tmp_path / "story.md"
    p.write_text(templating.render_story_skeleton("做一个X", "2026-07-31"), encoding="utf-8")
    return p
...
def _design_doc(tmp_path: Path, kind: str, name: str) -> Path:
    ...
    p = tmp_path / name
    p.write_text(
        re.sub(r"<!--.*?-->", "", templating.load_template(kind), flags=re.S), encoding="utf-8"
    )
    return p
...
def test_design_kinds_template_conformant(tmp_path):
    ...
    assert check_template(_design_doc(tmp_path, "architecture", "architecture.md")) == []
    assert check_template(_design_doc(tmp_path, "interfaces", "interfaces.md")) == []
...
def test_architecture_requires_scaffold_manifest_section(tmp_path):
    ...
    p.write_text(text.replace("## 2. Scaffold 宣言", "Scaffold 宣言"), encoding="utf-8")
    issues = check_template(p)
...
def test_design_templates_carry_no_blockquotes():
    ...
    for kind in ("architecture", "interfaces", "test-plan"):
        ...
        assert not [ln for ln in text.splitlines() if ln.lstrip().startswith(">")], kind
...
def test_check_template_leftover_guidance_blockquote_fails(tmp_path):
    ...
    p.write_text(
        text[:idx] + "> **When needed**: Required when ...\n\n" + text[idx:], encoding="utf-8"
    )
    issues = check_template(p)
    assert any(
        i.startswith("line:")
        and "template guidance blockquote left in doc" in i
        and "'When needed'" in i
        for i in issues
    )
...
def test_fake_design_trio_has_no_blockquotes(tmp_path):
    ...
    for name in DESIGN_DOCS:
        ...
        assert not [ln for ln in text.splitlines() if ln.lstrip().startswith(">")], name
...
def test_trac_subcommands_pin_parity_with_cli_usage():
    ...
    names = {
        m.group(1)
        for part in re.sub(r"<[^>]*>", "", USAGE.removeprefix("usage: trac ")).split("|")
...
def test_design_doc_unknown_trac_subcommand_fails(tmp_path):
    ...
    issues = check_template(p)
    assert any(
        i.startswith(f"line:{line_no} ") and "unknown trac subcommand 'agent'" in i for i in issues
    )
...
def test_legacy_story_missing_demand_desc_fails(tmp_path):
    ...
    p.write_text(
        text.replace(
            "## 3. 需求描述（规划层，待 M-STORY 展开）", "需求描述（规划层，待 M-STORY 展开）"
        ),
        encoding="utf-8",
    )
...
def test_discussion_diff_modify_raw_blockquote_plus_canonical_rejects(tmp_path):
    ...
    repo, doc = _setup_disc_repo(tmp_path, _BASE_DOC)
    current = (
        _BASE_DOC.replace("> Raw blockquote from human input.", "> Modified raw blockquote.")
        + "\n> **Sage:** comment.\n"
    )
...
def test_discussion_diff_body_text_change_rejects(tmp_path):
    ...
    repo, doc = _setup_disc_repo(tmp_path, _BASE_DOC)
    current = (
        _BASE_DOC.replace("Some content here.", "Changed content.") + "\n> **Sage:** comment.\n"
    )
    doc.write_text(current, encoding="utf-8")
    assert not is_discussion_diff(repo, doc, "HEAD")
...
def test_discussion_delta_trailing_space_on_body_rejects():
    ...
    base = _BASE_DOC
    current = (
        _BASE_DOC.replace("Some content here.\n", "Some content here.   \n")
        + "\n> **Sage:** comment.\n"
    )
    assert not is_discussion_delta(base, current)
