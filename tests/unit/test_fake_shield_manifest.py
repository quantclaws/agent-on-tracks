...
_TASKS_MULTI = [
    {"ac_id": "AC-FR0010-01", "layers": ["integration", "e2e"], "if_ids": ["IF-MTEST-001"]},
    {"ac_id": "AC-FR0020-01", "layers": ["integration"], "if_ids": ["IF-MTEST-001"]},
]
...
def _act(repo, tasks, evidence=None):
    return FakeBackend(repo, "v0.4").act(
        ...
        None,
        None,
        _assignment(tasks, evidence),
    )


def _manifest_paths(result):
    return [e["path"] for e in result.get("artifact_manifest", {}).get("include", [])]
...
def test_first_write_manifest_includes_all_created_test_files(tmp_path):
    ...
    assert _manifest_paths(result) == ["tests/integration/test_ac_fr0010_01.py"]
...
def test_noop_identical_rewrite_manifest_is_empty(tmp_path):
    ...
    assert _manifest_paths(first) == ["tests/integration/test_ac_fr0010_01.py"]

    second = _act(tmp_path, _TASKS_SINGLE)

    assert second["status"] == "done"
    assert _manifest_paths(second) == [], (
        "identical rewrite must not claim unchanged files; got: " + repr(_manifest_paths(second))
    )
...
def test_revision_redispatch_manifest_includes_modified_files(tmp_path):
    ...
    assert second["status"] == "done"
    assert _manifest_paths(second) == ["tests/integration/test_ac_fr0010_01.py"], (
        "revision marker modifies the file; it must appear in the manifest"
    )
    second_content = test_file.read_text(encoding="utf-8")
    assert len(second_content) > len(first_content), "revision marker must have appended content"
...
def test_manifest_paths_are_sorted_unique_and_repo_relative(tmp_path):
    ...
    assert len(paths) == len(set(paths)), f"paths must be unique; got {paths}"
    assert all(p.startswith("tests/") for p in paths), f"paths must be under tests/; got {paths}"
    assert all(not p.startswith("/") for p in paths), f"paths must be repo-relative; got {paths}"
