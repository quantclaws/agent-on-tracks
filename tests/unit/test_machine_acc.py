...
COMMITTED = ("acceptance.committed", {"commit_sha": "c1", "acceptance_sha": "s1",
                                      "final": False})
...
def test_human_review():
    ...
    exited = state_of(COMMITTED, ("lex.verdict", {"verdict": "pass"}),
                      ("human.review", {"action": "no_comment"}))
    assert exited.substate == "EXIT" and exited.awaiting is None
    respond = state_of(COMMITTED, ("lex.verdict", {"verdict": "pass"}),
...
def test_exit_seals_acceptance():
    ...
    s = state_of(COMMITTED, ("lex.verdict", {"verdict": "pass"}),
                 ("human.review", {"action": "no_comment"}))
    cmd = decide(s)
    assert cmd.kind == "validate_document"
    assert cmd.params == {"doc": "acceptance.md",
                          "checks": ["template", "discussion_ready"]}
    sealed = state_of(COMMITTED, ("lex.verdict", {"verdict": "pass"}),
                      ("human.review", {"action": "no_comment"}),
...
def test_trace_fail_redispatch_then_escalate():
    ...
    failed = state_of(("verdict.failed", {"check": "trace", "reason": "orphan",
                                          "attempt": 1}))
    cmd = decide(failed)
    assert cmd.kind == "dispatch_agent" and cmd.params["evidence"]["check"] == "trace"
    escalated = state_of(("verdict.failed", {"check": "trace", "reason": "orphan",
                                             "attempt": 3}))
...
def test_rollback_targets():
    ...
    for target in ("M-SPEC", "M-STORY"):
        s = state_of(COMMITTED,
                     ("stage.rolled_back", {"from_stage": "M-ACC",
                                            "to_stage": target,
                                            "reason": "trace gap"}))
